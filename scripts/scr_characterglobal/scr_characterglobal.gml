function characterspr()
{
	// sprites
	spr_idle = spr_pizzelle_idle;
	spr_move = spr_pizzelle_walk;
	spr_angryidle = spr_pizzelle_3hpidle;
	spr_angrywalk = spr_pizzelle_3hpwalk;
	spr_crawl = spr_pizzelle_crawl;
	spr_hurt = spr_pizzelle_hurt;
	spr_jump = spr_pizzelle_jump;
	spr_jump2 = spr_pizzelle_jump;
	spr_fall = spr_pizzelle_fall;
	spr_fall2 = spr_pizzelle_fall;
	spr_bodyslamstart = spr_pizzelle_groundpoundprep;
	spr_bodyslamfall = spr_pizzelle_groundpound;
	spr_bodyslamland = spr_pizzelle_groundpoundland;
	spr_bombdropstart = spr_pizzelle_bodyslamstart;
	spr_bombdropfall = spr_pizzelle_bodyslamfall;
	spr_bombdropland = spr_pizzelle_bodyslamland;
	spr_crouch = spr_pizzelle_crouch;
	spr_crouchjump = spr_pizzelle_crouchjump;
	spr_crouchfall = spr_pizzelle_crouchfall;
	spr_couchstart = spr_pizzelle_crouchstart;
	spr_splat = spr_pizzelle_splat;
	spr_bump = spr_pizzelle_bump;
	spr_land = spr_pizzelle_land;
	spr_land2 = spr_pizzelle_land2;
	spr_lookdoor = spr_pizzelle_lookdoor;
	spr_walkfront = spr_pizzelle_walkfront;
	spr_victory = spr_pizzelle_entergate;
	spr_Ladder = spr_pizzelle_ladder;
	spr_laddermove = spr_pizzelle_ladderup;
	spr_ladderdown = spr_pizzelle_ladderdown;
	spr_keyget = spr_pizzelle_keyget;
	spr_crouchslip = spr_pizzelle_machslide;
	spr_slippingonice = spr_pizzelle_slippingonice;
	spr_mach1 = spr_pizzelle_mach1;
	spr_mach2 = spr_pizzelle_mach2;
	spr_mach3player = spr_pizzelle_mach3;
	spr_walljumpstart = spr_pizzelle_secondjump1
	spr_walljumpend = spr_pizzelle_secondjump2
	spr_suplexbump = spr_pizzelle_bump
	spr_mach2ceiling = spr_pizzelle_climbceilingmach2;
	spr_mach3ceiling = spr_pizzelle_climbceilingmach3;
	spr_machdashpad = spr_pizzelle_dashpad;
	spr_rollgetup = spr_pizzelle_rollgetup;
	spr_mach3jump = spr_pizzelle_mach3jump;
	spr_mach3hit = spr_pizzelle_mach3
	spr_mach3hitwall = spr_pizzelle_mach3hitwall;
	spr_mach3roll = spr_pizzelle_machroll
	spr_mach3rollstart = spr_pizzelle_machroll
	spr_crazyrun = spr_pizzelle_crazyrun;
	spr_secondjump1 = spr_pizzelle_secondjump1;
	spr_secondjump2 = spr_pizzelle_secondjump2;
	spr_machslidestart = spr_pizzelle_machslidestart;
	spr_machslide = spr_pizzelle_machsliding;
	spr_machslideend = spr_pizzelle_machslideend;
	spr_grindrail = spr_pizzelle_grindrail;
	spr_machslideboost = spr_pizzelle_machslideboost;
	spr_machslideboostFallStart = spr_pizzelle_machslideboostFallStart;
	spr_machslideboostFall = spr_pizzelle_machslideboostFall;
	spr_machslideboost3 = spr_pizzelle_machslideboost3;
	spr_machslideboost3FallStart = spr_pizzelle_machslideboost3FallStart;
	spr_machslideboost3Fall = spr_pizzelle_machslideboost3Fall;
	spr_land2 = spr_pizzelle_land2;
	spr_punch = spr_pizzelle_punch;
	spr_backkick = spr_pizzelle_backkick;
	spr_shoulder = spr_pizzelle_shoulder;
	spr_uppunch = spr_pizzelle_uppunch;
	spr_stomp = spr_pizzelle_stomp;
	spr_stompprep = spr_pizzelle_stompprep;
	spr_crouchslide = spr_pizzelle_machslide;
	spr_crouch = spr_pizzelle_crouch;
	spr_bump = spr_pizzelle_bump;
	spr_victory = spr_pizzelle_entergate;
	spr_hurt = spr_pizzelle_hurt;
	spr_climbwall = spr_pizzelle_climbwall;
	spr_climbdownwall = spr_pizzelle_climbdownwall;
	spr_grab = spr_pizzelle_grab;
	spr_mach2jump = spr_pizzelle_mach2jump;
	spr_longjump = spr_pizzelle_mach2jump
	spr_longjumpend = spr_pizzelle_mach2jump
	spr_trick = spr_pizzelle_trick
	spr_Timesup = spr_pizzelle_timeup;
	spr_deathstart = spr_pizzelle_deathstart;
	spr_deathend = spr_pizzelle_deathend;
	spr_machpunch1 = spr_pizzelle_machpunch1;
	spr_machpunch2 = spr_pizzelle_machpunch2;
	spr_hurtjump = spr_pizzelle_hurt;
	spr_entergate = spr_pizzelle_entergate;
	spr_gottreasure = spr_pizzelle_entergate;
	spr_bossintro = spr_pizzelle_bossintro;
	spr_hurtidle = spr_pizzelle_hurtidle;
	spr_hurtwalk = spr_pizzelle_hurtwalk;
	spr_suplexmash1 = spr_pizzelle_suplexmash1;
	spr_suplexmash2 = spr_pizzelle_suplexmash2;
	spr_suplexmash3 = spr_pizzelle_suplexmash3;
	spr_suplexmash4 = spr_pizzelle_suplexmash4;
	spr_blockbreak1 = spr_pizzelle_blockbreak1
	spr_blockbreak2 = spr_pizzelle_blockbreak2
	spr_blockbreak3 = spr_pizzelle_blockbreak3
	spr_blockbreak4 = spr_pizzelle_blockbreak4
	spr_blockbreak5 = spr_pizzelle_blockbreak5
	spr_blockbreak6 = spr_pizzelle_blockbreak6
	spr_blockbreak7 = spr_pizzelle_blockbreak7
	spr_tackle = spr_pizzelle_tackle;
	spr_airdash1 = spr_pizzelle_airdash1;
	spr_airdash2 = spr_pizzelle_airdash2;
	spr_keyget = spr_pizzelle_keyget;
	spr_idle1 = spr_pizzelle_idleanim1;
	spr_idle2 = spr_pizzelle_idleanim2;
	spr_idle3 = spr_pizzelle_idleanim3;
	spr_idle4 = spr_pizzelle_idleanim4;
	spr_parry1 = spr_pizzelle_parry1;
	spr_parry2 = spr_pizzelle_parry2;
	spr_parry3 = spr_pizzelle_parry3;
	spr_supertaunt1 = spr_pizzelle_supertaunt1;
	spr_supertaunt2 = spr_pizzelle_supertaunt2;
	spr_supertaunt3 = spr_pizzelle_supertaunt3;
	spr_suplexdash = spr_pizzelle_suplexdash;
	spr_piledriver = spr_pizzelle_piledriver;
	spr_piledriverland = spr_pizzelle_piledriverland;
	spr_charge = spr_pizzelle_charge;
	spr_taunt = spr_pizzelle_taunt;
	spr_selectpal = spr_pizzelle_selectpal;
	spr_caneidle = spr_pizzelle_caneidle;
	spr_caneslam = spr_pizzelle_caneslam;
	spr_canewalk = spr_pizzelle_canewalk;
	spr_canesuplex = spr_pizzelle_canesuplex;
	spr_canehit = spr_pizzelle_canehit;
	spr_uppizzabox = spr_pizzelle_uppizzabox;
	spr_downpizzabox = spr_pizzelle_downpizzabox;
	spr_tumblestart = spr_pizzelle_tumblestart;
	spr_tumble = spr_pizzelle_tumble;
	spr_tumbleend = spr_pizzelle_tumbleend;
	spr_machroll =  spr_pizzelle_machroll;
	spr_dive = spr_pizzelle_dive;
	spr_cottonidle = spr_cotton_idle;
	spr_escapeidle = spr_pizzelle_escapeidle;
	spr_timesupidle = spr_pizzelle_timesupidle;
	spr_piledriverstart = spr_pizzelle_piledriverstart;
	spr_haulingstart = spr_pizzelle_haulingstart;
	spr_haulingidle = spr_pizzelle_haulingidle;
	spr_haulingfall = spr_pizzelle_haulingfall;
	spr_haulingjump = spr_pizzelle_haulingjump;
	spr_haulingland = spr_pizzelle_haulingland;
	spr_haulingwalk = spr_pizzelle_haulingwalk;
	spr_winding = spr_pizzelle_winding;
	spr_fudgeidle = spr_pizzelle_fudgeidle;
	spr_danceidle = spr_pizzelle_danceidle;
	spr_uppercutbegin = spr_pizzelle_uppercutbegin;
	spr_uppercutend = spr_pizzelle_uppercutend;
	spr_uppercutfinishingblow = spr_pizzelle_uppercutfinishingblow;
	spr_idlevomit = spr_pizzelle_idlevomit;
	spr_idlevomitblood = spr_pizzelle_idlevomitblood;
	spr_crouchshoot = spr_pizzelle_crouchshoot;
	spr_skatecrouch = spr_pizzelle_skatecrouch;
	spr_facehurtup = spr_pizzelle_facehurtup;
	spr_facehurt = spr_pizzelle_facehurt;
	spr_breakdance = spr_pizzelle_breakdance;
	spr_shotgun_player = spr_peppino_shotgun;
	spr_canefall = spr_pizzelle_canefall;
	spr_breakdancebeach = spr_pizzelle_breakdancebeach;
	spr_swingding = spr_pizzelle_swingding;
	spr_swingdingend = spr_pizzelle_swingdingend;
	spr_downslopes = spr_pizzelle_downslopes;
	spr_upslopes = spr_pizzelle_upslopes;
	spr_ceilingcrash = spr_pizzelle_ceilingcrash;
	spr_machfreefall = spr_pizzelle_machfreefall;
	spr_throwDonut = spr_pizzelle_throwDonut;
	spr_superjumpprep = spr_pizzelle_superjumpprep;
	spr_superjumppreplight = spr_pizzelle_superjumppreplight;
	spr_superjumpright = spr_pizzelle_superjumpright;
	spr_superjumpleft = spr_pizzelle_superjumpleft;
	spr_superjump = spr_pizzelle_superjump;
	spr_superjump_cancel = spr_pizzelle_superjump_cancel;
	spr_superjump_cancelgroundedmach4 = spr_pizzano_superjump_cancelmach4grounded;
	spr_superjump_cancelgrounded = spr_pizzano_mach3;
	spr_superjump_cancelprep = spr_pizzelle_superjump_cancelprep;
	spr_superjumpland = spr_pizzelle_superjumpland;
	spr_suplexdashCancel = spr_pizzelle_suplexdashCancel;
	spr_outofcontrolfall = spr_pizzelle_outofcontrolfall;
	spr_machtumble = spr_pizzelle_machtumble;
	spr_skatejumpstart = spr_pizzelle_skatejumpstart;
	spr_facestomp = spr_pizzelle_facestomp;
	spr_freefall =  spr_pizzelle_freefall;
	spr_freefall2 = spr_pizzelle_freefall2;
	spr_freefallland = spr_pizzelle_freefallland;
	spr_donutSlam_intro = spr_pizzelle_donutSlam_intro;
	spr_candytransitionup = spr_pizzelle_candytransitionup;
	spr_candyup = spr_pizzelle_candyup;
	spr_spitair = spr_coneboy_spitair;
	spr_djump = spr_pizzano_djump;
	spr_kungfuair1start = spr_pizzano_kungfuair1start;
	spr_kungfuair2start = spr_pizzano_kungfuair2start;
	spr_kungfuair3start = spr_pizzano_kungfuair3start;
	spr_kungfu1 = spr_pizzelle_kungfu;
	spr_kungfu2 = spr_pizzelle_kungfu;
	spr_kungfu3 = spr_pizzelle_kungfu;
	spr_kungfu4 = spr_pizzelle_kungfu;
	spr_kungfu5 = spr_pizzelle_kungfu;
	spr_kungfu6 = spr_pizzelle_kungfu;
	spr_twirl = spr_pizzano_twirl;
	spr_small_destroyable = spr_smallbreak;
	spr_large_destroyable = spr_bigbreak;
	spr_large_destroyable_hard = spr_bigbreakhard;
	spr_small_destroyable_collect = spr_smallbreak2;
	spr_large_destroyable_collect = spr_bigbreak2;
	spr_metal_destroyable = spr_metalblock;
	spr_small_destroyable_escape = spr_smallbreakEscape;
	spr_large_destroyable_escape = spr_bigbreakEscape;
	spr_small_destroyable_collect_escape = spr_smallbreak2Escape;
	spr_large_destroyable_collect_escape = spr_bigbreak2Escape;
	spr_metal_destroyable_escape = spr_metalblockEscape;
	spr_metal_destroyable_hard = spr_metalblockhard;
	spr_smallcollect1 = spr_collect1;
	spr_smallcollect2 = spr_collect2;
	spr_smallcollect3 = spr_collect3;
	spr_smallcollect4 = spr_collect4;
	spr_smallcollect5 = spr_collect5;
	spr_bigcollect1 = spr_bigcollect1P;
	spr_bigcollect2 = spr_bigcollect2P;
	spr_bigcollect3 = spr_bigcollect3P;
	spr_bigcollect4 = spr_bigcollect4P;
	spr_bigcollect5 = spr_bigcollect5P;
	spr_bigcollect6 = spr_bigcollect6P;
	spr_massivecollect1 = spr_giantcollect1;
	spr_massivecollect2 = spr_giantcollect2;
	spr_massivecollect3 = spr_giantcollect3;
	spr_massivecollect4 = spr_giantcollect4;
	spr_smallcollectescape1 = spr_collectescape;
	spr_bigcollectescape1 = spr_bigcollectescape;
	spr_collectslice1 = spr_collectslice;
	spr_confectcage = spr_confecticage;
	spr_confectcage_hop = spr_confecticage_hop;
	spr_confectcage_help = spr_confecticage_help;
	spr_gummyharry_idle = spr_harryNEW;
	spr_gummyharry_dead = spr_harrydeadNEW;
	spr_player_rankE = spr_pizzelle_rankP;
	spr_player_rankP =spr_pizzelle_rankP;
	spr_player_rankS = spr_pizzelle_rankS;
	spr_player_rankA = spr_pizzelle_rankA;
	spr_player_rankB = spr_pizzelle_rankB;
	spr_player_rankC = spr_pizzelle_rankC;
	spr_player_rankD = spr_pizzelle_rankD;
	spr_rankwait = spr_pizzelle_rankwait;
	spr_rankbubble_E = spr_rankbubble_e;
	spr_rankbubble_Efilled = spr_rankbubble_efilled;
	spr_rankbubble_Eempty = spr_rankbubble_e_empty;
	spr_rankbubble_Efont = spr_rankbubble_e_font;
	spr_rankbubble_P = spr_rankbubble_p;
	spr_rankbubble_Pfilled = spr_rankbubble_pfilled;
	spr_rankbubble_S = spr_rankbubble_s;
	spr_rankbubble_Sfilled = spr_rankbubble_sfilled;
	spr_rankbubble_A = spr_rankbubble_a;
	spr_rankbubble_Afilled = spr_rankbubble_afilled;
	spr_rankbubble_B = spr_rankbubble_b;
	spr_rankbubble_Bfilled = spr_rankbubble_bfilled;
	spr_rankbubble_C = spr_rankbubble_c;
	spr_rankbubble_Cfilled = spr_rankbubble_cfilled;
	spr_rankbubble_D = spr_rankbubble_d;
	spr_rankbubble_Dfilled = spr_rankbubble_dfilled;
	spr_lungehit = spr_pizzelle_lungehit;
	spr_confecti5_rank = spr_confecti5rank;
	spr_confecti4_rank = spr_confecti4rank;
	spr_confecti3_rank = spr_confecti3rank;
	spr_confecti2_rank = spr_confecti2rank;
	spr_confecti1_rank = spr_confecti1rank;
	spr_lapjanitorrank = spr_rankotherjanitor;
	spr_rudejanitorrank = spr_rankrudejanitor;
	spr_cakerank = spr_rankcake;
	spr_confecti5_appear = spr_candy_appear;
	spr_confecti5_idle = spr_candy_idle;
	spr_confecti5_walk = spr_candy_walk;
	spr_confecti5_panic = spr_candy_panic;
	spr_confecti5_panicWalk = spr_candy_panicWalk;
	spr_confecti5_taunt = spr_candy_taunt;
	spr_confecti4_appear = spr_gummyworm_appear;
	spr_confecti4_idle = spr_gummyworm_idle;
	spr_confecti4_walk = spr_gummyworm_walk;
	spr_confecti4_panic = spr_gummyworm_panic;
	spr_confecti4_panicWalk = spr_gummyworm_panicWalk;
	spr_confecti4_taunt = spr_gummyworm_taunt;
	spr_confecti3_appear = spr_crack_appear;
	spr_confecti3_idle = spr_crack_idle;
	spr_confecti3_walk = spr_crack_run;
	spr_confecti3_panic = spr_crack_panic;
	spr_confecti3_panicWalk = spr_crack_panicWalk;
	spr_confecti3_taunt = spr_crack_taunt;
	spr_confecti2_appear = spr_chocolate_appear;
	spr_confecti2_idle = spr_chocolate_idle;
	spr_confecti2_walk = spr_chocolate_walk;
	spr_confecti2_panic = spr_chocolate_panic;
	spr_confecti2_panicWalk = spr_chocolate_panicWalk;
	spr_confecti2_taunt = spr_chocolate_taunt;
	spr_confecti1_appear = spr_marshmallow_appear;
	spr_confecti1_idle = spr_marshmellow_idle;
	spr_confecti1_walk = spr_marshemellow_run;
	spr_confecti1_panic = spr_marshmellow_panic;
	spr_confecti1_panicWalk = spr_marshmellow_panicWalk;
	spr_confecti1_taunt = spr_marshmellow_taunt;
	spr_janitor_appear = spr_rudejanitor_appear;
	spr_janitor_idle = spr_rudejanitor_idle;
	spr_janitor_walk = spr_rudejanitor_walk;
	spr_janitor_panic = spr_rudejanitor_panicidle;
	spr_janitor_panicWalk = spr_rudejanitor_panicwalk;
	spr_janitor_taunt = spr_rudejanitor_taunt;
	spr_lapjanitor_appear = spr_janitor2_jump;
	spr_lapjanitor_idle = spr_janitor2_idle;
	spr_lapjanitor_walk = spr_janitor2_run;
	spr_lapjanitor_panic = spr_janitor2_idle;
	spr_lapjanitor_panicWalk = spr_janitor2_run;
	spr_lapjanitor_taunt = spr_janitor2_taunt;
	spr_large_destroyable_oneway = spr_onewayblock;
	spr_dancewalk = spr_pizzelle_dancewalk;
	spr_minecart_player = spr_pizzelle_minecart;
	spr_minecartfast = spr_pizzelle_minecartfast;
	spr_minecartfastland = spr_pizzelle_minecartfastland;
	spr_minecartland = spr_pizzelle_minecartland;
	spr_minecartjump = spr_pizzelle_minecartjump;
	spr_minecartfall = spr_pizzelle_minecartfall;
	spr_mask = spr_pizzelle_mask;
	spr_fireass = spr_pizzelle_fireass;
	spr_candyidle = spr_pizzelle_candyidle;
	spr_candybegin = spr_pizzelle_candybegin;
	spr_bottlerocketstart = spr_pizzelle_bottlerocketstart;
	spr_hookvert = spr_pizzelle_hookvert
	spr_hookhoriz = spr_pizzelle_hookhoriz
	spr_lapvisual_player = spr_lapvisual
	spr_lapbg_player = spr_lapbg
	img_featuringme = 0;
	spr_coneball_player = spr_coneball_new
	spr_coneball_melting_player = spr_coneball_melting
	spr_stylebar = spr_heatmeter
	spr_stylebarunder = spr_heatmeterunder
	spr_collecthud = spr_cakehud
	spr_crankhud = spr_cranktopping
	spr_brankhud = spr_branktopping
	spr_arankhud = spr_aranktopping
	spr_srankhud = spr_sranktopping
	spr_gatehud = spr_gatecake
	spr_combotitle = spr_combotitles
	spr_combotitle_BR = spr_combotitles_BR
	spr_combotitle_ES = spr_combotitles_ES
	spr_combotitle_truly = spr_combotruly
	spr_roomnamehud = spr_roomnamebg
	spr_lapportal_create = spr_lappingportal_create
	spr_lapportal_idle = spr_lappingportal_idle
	spr_lapportal_enter = spr_lappingportal_enter
	spr_lapportal_exit = spr_lappingportal_exit
	spr_heatmeterpal = spr_heatpal
	spr_escapepopup = spr_sugarrush
	spr_fireassend = spr_pizzelle_fireassend
	spr_fireassgrab = spr_pizzelle_fireassgrab
	spr_fireassstart = spr_pizzelle_fireass
	spr_donutSlam_land = spr_pizzelle_donutSlam_land
	spr_skatedive = spr_pizzelle_skatedive
	spr_skatedoublejump = spr_pizzelle_skatedoublejump
	spr_skatedoublejumpstart = spr_pizzelle_skatedoublejumpstart
	spr_skatedrift = spr_pizzelle_skatedrift
	spr_skatefall = spr_pizzelle_skatefall
	spr_skateidle = spr_pizzelle_skateidle
	spr_skatejump = spr_pizzelle_skatejump
	spr_skateland = spr_pizzelle_skateland
	spr_skateslowwalk = spr_pizzelle_skateslowwalk
	spr_skatespin = spr_pizzelle_skatespin
	spr_skatewalk = spr_pizzelle_skatewalk
	spr_skatewalljump = spr_pizzelle_skatewalljump
	spr_skatewalljumpstart = spr_pizzelle_skatewalljumpstart
	spr_skatewallrun = spr_pizzelle_skatewallrun
	spr_frothstuck = spr_pizzelle_frothstuck
	spr_frothstuck_start = spr_pizzelle_frothstuck_start
	spr_player_gumball = spr_pizzelle_gumball
	spr_seacream_fall = spr_pizzelle_seacream_fall
	spr_seacream_idle = spr_pizzelle_seacream_idle
	spr_seacream_jump = spr_pizzelle_seacream_jump
	spr_seacream_stick = spr_pizzelle_seacream_stick
	spr_seacream_walk = spr_pizzelle_seacream_walk
	spr_seacream_walljump = spr_pizzelle_seacream_walljump
	spr_ufoFloat = spr_pizzelle_ufoFloat
	spr_ufoFloatLow = spr_pizzelle_ufoFloatLow
	spr_ufoFloatSpeedUp = spr_pizzelle_ufoFloatSpeedUp
	spr_ufojump = spr_pizzelle_ufojump
	spr_uforoll = spr_pizzelle_uforoll
	spr_ufoShoot = spr_pizzelle_ufoShoot
	spr_ufoShootHold = spr_pizzelle_ufoShootHold
	spr_faucetpull =  spr_pizzelle_faucetpull
	spr_player_bottlerocket = spr_pizzelle_bottlerocket
	spr_bottlerocket_grounded = spr_pizzelle_bottlerocket_grounded
	spr_bottlerocket_turn = spr_pizzelle_bottlerocket_turn
	spr_slipnslidestart = spr_pizzelle_slipnslidestart
	spr_slipnslide = spr_pizzelle_slipnslide
	spr_squished = spr_pizzelle_squished
	spr_catched = spr_pizzelle_catched
	spr_honeystuck = spr_pizzelle_honeystuck
	spr_geyser = spr_pizzelle_geyser
	spr_drown_jump = spr_pizzelle_drown_jump
	spr_drown = spr_pizzelle_drown
	spr_donutget = spr_pizzelle_donutget
	spr_player_cookiemount = spr_pizzelle_cookiemount
	spr_cookiemount_dash = spr_pizzelle_cookiemount_dash
	spr_cookiemount_idle = spr_pizzelle_cookiemount_idle
	spr_cookiemount_skid = spr_pizzelle_cookiemount_skid
	spr_cookiemountfireass = spr_pizzelle_cookiemountfireass
	spr_cookiemountfireassend = spr_pizzelle_cookiemountfireassend
	spr_cookiemountballonstart = spr_pizzelle_cookiemountballonstart
	spr_cookiemountballon = spr_pizzelle_cookiemountballon
	spr_donutSlam = spr_pizzelle_donutSlam
	spr_levelcomplete = spr_pizzelle_levelcomplete
	spr_bossintro = spr_pizzelle_bossintro
	spr_machtwirl = spr_pizzano_machtwirl
	spr_slipnslideend = spr_pizzelle_slipnslideend
	spr_cottoncreator = spr_cottonmaker
	spr_cotton_platform = spr_cottonplatform_tiled
	spr_electrocuted = spr_pizzelle_electrocuted
	if global.newplayeranim == 1
	{
		spr_mach1 = spr_pizzelle_mach1_new;
		spr_mach2 = spr_pizzelle_mach2_new;
		spr_mach3player = spr_pizzelle_mach3_new;
		spr_crazyrun = spr_pizzelle_crazyrun_new;
		spr_angryidle = spr_pizzelle_3hpidle_new
		spr_angrywalk = spr_pizzelle_3hpwalk_new;
		spr_splat = spr_pizzelle_splat_new;
		spr_fireass = spr_pizzelle_fireass_new
		spr_fireassgrab = spr_pizzelle_fireassgrab_new
		spr_fireassstart = spr_pizzelle_fireassstart_new
		spr_fireassend = spr_pizzelle_fireassend
		spr_mach3hit = spr_pizzelle_mach3hit_new
		spr_machslideboost3 = spr_pizzelle_mach3boost_new
		spr_climbwall = spr_pizzelle_climbwall_new
		spr_longjump = spr_pizzelle_longjump_new
		spr_longjumpend = spr_pizzelle_longjumpfall_new
		spr_rollgetup = spr_pizzelle_rollgetup_new
		spr_mach3rollstart = spr_pizzelle_mach3rollstart_new
		spr_mach3roll = spr_pizzelle_mach3roll_new
		spr_suplexbump = spr_pizzelle_splat_new
		spr_mach3jump = spr_pizzelle_mach3jump_new;
		spr_winding = spr_pizzelle_winding_new
		spr_mach3ceiling = spr_pizzelle_ceilingmach_new
		spr_machslideboostFallStart = spr_pizzelle_machslideboostFall_new;
		spr_machslideboostFall = spr_pizzelle_machslideboostFall_new;
	}
	else if global.newplayeranim == 2
	{
		spr_mach1 = spr_pizzelle_mach1_alt;
		spr_mach2 = spr_pizzelle_mach2_alt;
		spr_mach3player = spr_pizzelle_mach3_alt;
		spr_crazyrun = spr_pizzelle_crazyrun_alt;
		spr_mach3ceiling = spr_pizzelle_climbceilingmach_alt
	}
	else if global.newplayeranim == 3
	{
		spr_jump = spr_pizzelle_jump_classic;
		spr_land = spr_pizzelle_land_classic;
		spr_land2 = spr_pizzelle_land2_classic;
		spr_Timesup = spr_pizzelle_timeup_classic;
		spr_mach2 = spr_pizzelle_mach2_classic;
		spr_secondjump1 = spr_pizzelle_secondjump1_classic;
		spr_secondjump2 = spr_pizzelle_secondjump2_classic;
		spr_mach2jump = spr_pizzelle_mach2flip_classic;
		spr_machpunch1 = spr_pizzelle_machpunch1_classic;
		spr_machpunch2 = spr_pizzelle_machpunch2_classic;
		spr_mach3jump = spr_pizzelle_mach3jump_classic;
		spr_blockbreak1 = spr_pizzelle_suplexmash1_classic;
		spr_blockbreak2 = spr_pizzelle_suplexmash2_classic;
		spr_blockbreak3 = spr_pizzelle_suplexmash3_classic;
		spr_blockbreak4 = spr_pizzelle_suplexmash4_classic;
		spr_blockbreak5 = spr_pizzelle_suplexmash1_classic
		spr_blockbreak6 = spr_pizzelle_suplexmash2_classic
		spr_blockbreak7 = spr_pizzelle_suplexmash3_classic
		spr_mach3player = spr_pizzelle_mach3_classic;
		spr_angryidle = spr_pizzelle_3hpidle_classic;
		spr_angrywalk = spr_pizzelle_3hpwalk_classic;
		spr_suplexdash = spr_pizzelle_suplexgrab_classic;
		spr_piledriverstart = spr_pizzelle_piledriverstart_classic;
		spr_piledriver = spr_pizzelle_piledriver_classic;
		spr_piledriverland = spr_pizzelle_piledriverland_classic;
		spr_haulingstart = spr_pizzelle_haulingstart_classic;
		spr_uppercutbegin = spr_pizzelle_uppercutbegin_classic;
		spr_uppercutend = spr_pizzelle_uppercutend_classic;
		spr_fireass = spr_pizzelle_fireass_classic;
		spr_cookiemount_skid = spr_pizzelle_cookiemount_skid_old
		spr_skateidle = spr_pizzelle_skateidle_old
	}
	if global.destroyables
	{
		spr_small_destroyable = spr_smallbreak_new
		spr_large_destroyable = spr_bigbreak_new
		spr_large_destroyable_oneway = spr_onewayblock_new
		spr_large_destroyable_hard = spr_bigbreakhard_new
	}
	// fonts
	font_lapcount = global.lapfont
	font_collect_new = global.candlefont
	font_collect = global.collectfont
	// audio
	mu_escapesong = mu_pizzelle_escape
	mu_lap2song = mu_pizzelle_lap
	mu_lap5song = mu_pizzelle_despairy
	mu_lap10song = mu_pizzelle_death
	sfx_lapbell = sfx_lapbells
	sfx_escapecollectbig = sound_bigescapeclock
	sfx_escapecollect = sound_escapeclock
	sfx_collectmassive = sound_pizzagot
	sfx_collectlarge = sound_points
	sfx_collect = sound_points
	sfx_taunt = "sound_taunt"
	taunt_upperrange = 7
	sfx_supertaunt = sound_supertaunt1
	switch (character)
	{
		case "N":
			spr_idle = spr_pizzano_idle;
			spr_move = spr_pizzano_walk;
			spr_angryidle = spr_pizzano_3hpidle;
			spr_angrywalk = spr_pizzano_3hpwalk;
			spr_crawl = spr_pizzano_crawl;
			spr_hurt = spr_pizzano_hurt;
			spr_jump = spr_pizzano_jump;
			spr_jump2 = spr_pizzano_jump;
			spr_fall = spr_pizzano_fall;
			spr_fall2 = spr_pizzano_fall;
			spr_bodyslamstart = spr_pizzano_bodyslamstart;
			spr_bodyslamfall = spr_pizzano_bodyslamfall;
			spr_bodyslamland = spr_pizzano_bodyslamland;
			spr_bombdropstart = spr_pizzano_bodyslamstart;
			spr_bombdropfall = spr_pizzano_bodyslamfall;
			spr_bombdropland = spr_pizzano_bodyslamland;
			spr_crouch = spr_pizzano_crouch;
			spr_crouchjump = spr_pizzano_crouchjump;
			spr_crouchfall = spr_pizzano_crouchfall;
			spr_couchstart = spr_pizzano_crouchstart;
			spr_land = spr_pizzano_land;
			spr_land2 = spr_pizzano_land;
			spr_lookdoor = spr_pizzano_lookdoor;
			spr_walkfront = spr_pizzano_walkfront;
			spr_victory = spr_pizzano_entergate;
			spr_Ladder = spr_pizzano_ladder;
			spr_laddermove = spr_pizzano_ladderup;
			spr_ladderdown = spr_pizzano_ladderdown;
			spr_crouchslip = spr_pizzano_crouchslide;
			spr_mach1 = spr_pizzano_mach1;
			spr_mach2 = spr_pizzano_mach2;
			spr_mach3player = spr_pizzano_mach3shoulder;
			spr_machdashpad = spr_pizzano_mach3jump;
			spr_rollgetup = spr_pizzano_crouchslide;
			spr_mach3jump = spr_pizzano_mach3jump;
			spr_mach3hit = spr_pizzano_mach3slam;
			spr_mach3hitwall = spr_pizzano_mach3hitwall;
			spr_mach3roll = spr_pizzano_crouchslide
			spr_mach3rollstart = spr_pizzano_crouchslide
			spr_crazyrun = spr_pizzano_crazyrun;
			spr_machslidestart = spr_pizzano_crouchslide;
			spr_machslide = spr_pizzano_crouchslide;
			spr_machslideend = spr_pizzano_crouchslide;
			spr_machslideboost = spr_pizzano_machslideboost;
			spr_machslideboost3 = spr_pizzano_mach3boost;
			spr_land2 = spr_pizzano_land;
			spr_shoulder = spr_pizzano_mach3shoulder;
			spr_crouchslide = spr_pizzano_crouchslide;
			spr_crouch = spr_pizzano_crouch;
			spr_victory = spr_pizzano_entergate;
			spr_hurt = spr_pizzano_hurt;
			spr_Timesup = spr_pizzano_timeup;
			spr_hurtjump = spr_pizzano_hurt;
			spr_entergate = spr_pizzano_entergate;
			spr_gottreasure = spr_pizzano_entergate;
			spr_parry1 = spr_pizzano_parry1;
			spr_parry2 = spr_pizzano_parry1;
			spr_parry3 = spr_pizzano_parry1;
			spr_supertaunt1 = spr_pizzano_supertaunt1;
			spr_supertaunt2 = spr_pizzano_supertaunt1;
			spr_supertaunt3 = spr_pizzano_supertaunt1;
			spr_taunt = spr_pizzano_taunt;
			spr_cottonidle = spr_cotton_idle;
			spr_superjumpprep = spr_pizzano_superjumpprep;
			spr_superjumppreplight = spr_pizzano_superjumpprep;
			spr_superjumpright = spr_pizzano_superjumpprep;
			spr_superjumpleft = spr_pizzano_superjumpprep;
			spr_superjump = spr_pizzano_superjump;
			spr_superjump_cancel = spr_pizzano_superjump_cancel;
			spr_superjump_cancelmach4 = spr_pizzano_superjump_cancelmach4;
			spr_superjump_cancelgrounded = spr_pizzano_mach3;
			spr_superjump_cancelgroundedmach4 = spr_pizzano_superjump_cancelmach4grounded;
			spr_superjump_cancelprep = spr_pizzano_superjump_cancelprep;
			spr_djump = spr_pizzano_djump;
			spr_kungfuair1start = spr_pizzano_kungfuair1start;
			spr_kungfuair2start = spr_pizzano_kungfuair2start;
			spr_kungfuair3start = spr_pizzano_kungfuair3start;
			spr_kungfu1 = spr_pizzano_kungfu1;
			spr_kungfu2 = spr_pizzano_kungfu2;
			spr_kungfu3 = spr_pizzano_kungfu3;
			spr_kungfu4 = spr_pizzano_kungfu4;
			spr_kungfu5 = spr_pizzano_kungfu5;
			spr_kungfu6 = spr_pizzano_kungfu6;
			spr_twirl = spr_pizzano_twirl;
			spr_smallcollect1 = spr_collect1;
			spr_smallcollect2 = spr_collect2;
			spr_smallcollect3 = spr_collect3;
			spr_smallcollect4 = spr_collect4;
			spr_smallcollect5 = spr_collect5;
			spr_bigcollect1 = spr_bigcollect1P;
			spr_bigcollect2 = spr_bigcollect2P;
			spr_bigcollect3 = spr_bigcollect3P;
			spr_bigcollect4 = spr_bigcollect4P;
			spr_bigcollect5 = spr_bigcollect5P;
			spr_bigcollect6 = spr_bigcollect6P;
			spr_player_rankE = spr_pizzano_rankP;
			spr_player_rankP = spr_pizzano_rankP;
			spr_player_rankS = spr_pizzano_rankS;
			spr_player_rankA = spr_pizzano_rankA;
			spr_player_rankB = spr_pizzano_rankB;
			spr_player_rankC = spr_pizzano_rankC;
			spr_player_rankD = spr_pizzano_rankD;
			spr_rankwait = spr_pizzano_rankwait;
			spr_skateidle = spr_pizzano_frost_idle
			spr_skateslowwalk = spr_pizzano_frost_walk
			spr_skatewalk = spr_pizzano_frost_walk
			spr_machtwirl = spr_pizzano_machtwirl
			spr_cottoncreator = spr_cottonmakerstormy
			spr_cotton_platform = spr_cottonplatform_stormy
			img_featuringme = 1
			if global.newplayeranim == 2
				spr_jump = spr_pizzano_jump_alt
			else if global.newplayeranim == 3
			{
				spr_mach1 = spr_pizzano_mach1_old
				spr_mach2 = spr_pizzano_mach2_old
				spr_mach3player = spr_pizzano_mach3_old
				spr_mach3jump = spr_pizzano_mach3jump_old;
				spr_mach3rollstart = spr_pizzano_crouchslide_old	
				spr_mach3roll = spr_pizzano_crouchslide_old
				spr_player_rankA = spr_pizzano_rankA_old;
				spr_player_rankB = spr_pizzano_rankB_old;
				spr_player_rankC = spr_pizzano_rankC_old;
			}
			// audio
			mu_escapesong = mu_pizzano_escape
			mu_lap2song = mu_pizzano_lap
			mu_lap5song = mu_pizzano_despairy
			mu_lap10song = mu_pizzano_death
			sfx_taunt = "sfx_tauntpizzano"
			taunt_upperrange = 8
			break;
		case "G":
			spr_idle = spr_gumbob_idle;
			spr_move = spr_gumbob_walk;
			spr_hurt = spr_gumbob_hurt;
			spr_jump = spr_gumbob_jump;
			spr_jump2 = spr_gumbob_jump;
			spr_fall = spr_gumbob_fall;
			spr_fall2 = spr_gumbob_fall;
			spr_hurt = spr_gumbob_hurt;
			spr_hurtjump = spr_gumbob_hurt;
			spr_idle1 = spr_gumbob_idle;
			spr_idle2 = spr_gumbob_idle;
			spr_idle3 = spr_gumbob_idle;
			spr_taunt = spr_gumbob_taunt;
			spr_cottonidle = spr_gumbob_cotton_idle;
			spr_propeller_start = spr_gumbob_propeller_start;
			img_featuringme = 2;
			spr_mach1 = spr_gumbob_mach3;
			spr_mach2 = spr_gumbob_mach3;
			spr_mach3player = spr_gumbob_mach3;
			spr_mach3jump = spr_pizzano_mach3jump;
			spr_mach3hitwall = spr_pizzano_mach3hitwall;
			spr_crazyrun = spr_gumbob_mach3;
			// fonts
			font_lapcount = global.lapfont
			font_collect_new = global.candlefont
			font_collect = global.collectfont
			// audio
			sfx_taunt = "sfx_tauntgumbob"
			taunt_upperrange = 1
			break;
		case "C":
			spr_idle = spr_coneboy_idle;
			spr_move = spr_coneboy_walk;
			spr_jump = spr_coneboy_jump;
			spr_jump2 = spr_coneboy_jump;
			spr_fall = spr_coneboy_fall;
			spr_hurt = spr_coneboy_hurt;
			spr_crouch = spr_coneboy_guard;
			spr_couchstart = spr_coneboy_guard;
			spr_mach1 = spr_coneboy_dash;
			spr_mach2 = spr_coneboy_dash;
			spr_breakdance = spr_coneboy_boogy4
			spr_mach3player = spr_coneboy_dash;
			spr_crazyrun = spr_coneboy_crazyrun;
			spr_machslide = spr_coneboy_slidekick;
			spr_taunt = spr_coneboy_taunt;
			spr_spitair = spr_coneboy_spitair;
			img_featuringme = 3;
			if (inhalingenemy == 1)
			{
				spr_idle = spr_coneboy_inhaledidle;
				spr_move = spr_coneboy_inhaledwalk;
				spr_jump = spr_coneboy_inhaledjump;
				spr_jump2 = spr_gumbob_jump;
				spr_fall = spr_coneboy_inhaledfall;
				spr_fall2 = spr_gumbob_fall;
			}
			else if (headless == 1)
			{
				spr_idle = spr_coneboy_idleheadless;
				spr_move = spr_coneboy_walkheadless;
				spr_jump = spr_coneboy_jumpheadless;
				spr_jump2 = spr_gumbob_jump;
				spr_fall = spr_coneboy_fallheadless;
				spr_fall2 = spr_gumbob_fall;
			}
			sfx_taunt = "sound_tauntextra"
			taunt_upperrange = 2
			break;
		case "S":
			spr_idle = spr_peppino_idle;
			spr_move = spr_peppino_move;
			spr_angryidle = spr_peppino_3hpidle;
			spr_angrywalk = spr_peppino_3hpwalk;
			spr_crawl = spr_peppino_crawl;
			spr_hurt = spr_peppino_hurt;
			spr_jump = spr_peppino_jump;
			spr_jump2 = spr_peppino_jump;
			spr_fall = spr_peppino_fall;
			spr_fall2 = spr_peppino_fall2;
			spr_bodyslamstart = spr_peppino_bodyslamstart;
			spr_bodyslamfall = spr_peppino_bodyslamfall;
			spr_bodyslamland = spr_peppino_bodyslamland;
			spr_bombdropstart = spr_peppino_poundcancel1;
			spr_bombdropfall = spr_peppino_poundcancel1;
			spr_bombdropland = spr_peppino_poundcancel2;
			spr_crouch = spr_peppino_crouch;
			spr_crouchjump = spr_peppino_crouchjump;
			spr_crouchfall = spr_peppino_crouchfall;
			spr_splat = spr_peppino_wallsplat;
			spr_bump = spr_peppino_bump;
			spr_suplexbump = spr_peppino_suplexbump
			spr_land = spr_peppino_land;
			spr_land2 = spr_peppino_land2;
			spr_lookdoor = spr_peppino_lookdoor;
			spr_walkfront = spr_peppino_walkfront;
			spr_victory = spr_peppino_entergate;
			spr_Ladder = spr_peppino_ladder;
			spr_laddermove = spr_peppino_laddermove;
			spr_ladderdown = spr_peppino_ladderdown;
			spr_keyget = spr_peppino_keyget;
			spr_crouchslip = spr_peppino_crouchslip;
			spr_slippingonice = spr_peppino_slipnslide;
			spr_mach1 = spr_peppino_mach1;
			spr_mach2 = spr_peppino_mach;
			spr_mach3player = spr_peppino_mach4
			spr_machdashpad = spr_peppino_dashpad;
			spr_rollgetup = spr_peppino_rollgetup;
			spr_mach3jump = spr_peppino_mach3jump;
			spr_mach3hitwall = spr_peppino_mach3hitwall;
			spr_trick = spr_peppino_rampjump
			spr_crazyrun = spr_peppino_crazyrun;
			spr_mach3hit = spr_peppino_mach3hit
			spr_secondjump1 = spr_peppino_secondjump1;
			spr_secondjump2 = spr_peppino_secondjump2;
			spr_walljumpstart = spr_peppino_walljumpstart
			spr_walljumpend = spr_peppino_walljumpend
			spr_machslidestart = spr_peppino_machslidestart;
			spr_machslide = spr_peppino_machslide;
			spr_machslideend = spr_peppino_machslideend;
			spr_grindrail = spr_peppino_grind;
			spr_machslideboost = spr_peppino_machslideboost;
			spr_machslideboostFall = spr_peppino_machslideboostfall;
			spr_machslideboost3 = spr_peppino_machslideboost3;
			spr_machslideboost3FallStart = spr_peppino_machslideboost3fall;
			spr_machslideboost3Fall = spr_peppino_machslideboost3fall;
			spr_land2 = spr_peppino_land2;
			spr_punch = spr_peppino_punch;
			spr_backkick = spr_peppino_backkick;
			spr_shoulder = spr_peppino_shoulder;
			spr_uppunch = spr_peppino_uppunch;
			spr_stomp = spr_peppino_stomp;
			spr_stompprep = spr_peppino_stompprep;
			spr_crouchslide = spr_peppino_machslide;
			spr_crouch = spr_peppino_crouch;
			spr_bump = spr_peppino_bump;
			spr_victory = spr_peppino_entergate;
			spr_hurt = spr_peppino_hurt;
			spr_climbwall = spr_peppino_climbwall;
			spr_grab = spr_peppino_grab;
			spr_mach2jump = spr_peppino_mach2jump;
			spr_longjump = spr_peppino_longjump
			spr_longjumpend =  spr_peppino_longjumpend
			spr_Timesup = spr_peppino_timesup;
			spr_deathstart = spr_peppino_deathstart;
			spr_deathend = spr_peppino_deathend;
			spr_machpunch1 = spr_peppino_machpunch1;
			spr_machpunch2 = spr_peppino_machpunch2;
			spr_hurtjump = spr_peppino_hurt;
			spr_entergate = spr_peppino_entergate;
			spr_gottreasure = spr_peppino_entergate;
			spr_bossintro = spr_peppino_bossintro;
			spr_hurtidle = spr_peppino_hurtidle;
			spr_hurtwalk = spr_peppino_hurtwalk;
			spr_suplexmash1 = spr_peppino_finishingblow1;
			spr_suplexmash2 =spr_peppino_finishingblow2;
			spr_suplexmash3 = spr_peppino_finishingblow3;
			spr_suplexmash4 = spr_peppino_finishingblow4;
			spr_blockbreak1 = spr_peppino_suplexmash1;
			spr_blockbreak2 = spr_peppino_suplexmash2;
			spr_blockbreak3 = spr_peppino_suplexmash3;
			spr_blockbreak4 = spr_peppino_suplexmash4;
			spr_blockbreak5 = spr_peppino_suplexmash5;
			spr_blockbreak6 = spr_peppino_suplexmash6;
			spr_blockbreak7 = spr_peppino_suplexmash7;
			spr_tackle = spr_peppino_tackle;
			spr_airdash1 = spr_peppino_airdash1;
			spr_airdash2 = spr_peppino_airdash2;
			spr_keyget = spr_peppino_keyget;
			spr_idle1 = spr_peppino_idledance;
			spr_idle2 = spr_peppino_idlefrown;
			spr_idle3 = spr_peppino_handgesture1;
			spr_idle4 = spr_peppino_handgesture2;
			spr_idle5 = spr_peppino_handgesture3;
			spr_idle6 = spr_peppino_handgesture4;
			idle_upperrange = 6
			spr_parry1 = spr_peppino_parry1;
			spr_parry2 = spr_peppino_parry2;
			spr_parry3 = spr_peppino_parry3;
			spr_supertaunt1 = spr_peppino_supertaunt1;
			spr_supertaunt2 = spr_peppino_supertaunt2;
			spr_supertaunt3 = spr_peppino_supertaunt3;
			spr_suplexdash = spr_peppino_suplexdash;
			spr_piledriver = spr_peppino_piledriver;
			spr_piledriverland = spr_peppino_piledriverland;
			spr_charge = spr_peppino_charge;
			spr_uppizzabox = spr_peppino_uppizzabox;
			spr_downpizzabox = spr_peppino_downpizzabox;
			spr_tumblestart = spr_peppino_tumblestart;
			spr_taunt = spr_peppino_taunt;
			spr_machroll =  spr_peppino_machroll;
			spr_mach3rollstart = spr_peppino_backslideland
			spr_mach3roll = spr_peppino_backslide
			spr_dive = spr_peppino_dive;
			spr_cottonidle = spr_cotton_idle;
			spr_escapeidle = spr_peppino_panic;
			spr_timesupidle = spr_peppino_timesup;
			spr_piledriverstart = spr_peppino_piledriver;
			spr_haulingstart = spr_peppino_haulingstart;
			spr_haulingidle = spr_peppino_haulingidle;
			spr_haulingfall = spr_peppino_haulingfall;
			spr_haulingjump = spr_peppino_haulingjump;
			spr_haulingland = spr_peppino_haulingland;
			spr_haulingwalk = spr_peppino_haulingwalk;
			spr_winding = spr_peppino_winding;
			spr_fudgeidle = spr_peppino_freezeridle;
			spr_danceidle = spr_peppino_dance;
			spr_uppercutbegin = spr_peppino_breakdanceuppercut;
			spr_uppercutend = spr_peppino_breakdanceuppercutend;
			spr_uppercutfinishingblow = spr_peppino_uppercutfinishingblow;
			spr_idlevomit = spr_peppino_idlevomit;
			spr_idlevomitblood = spr_peppino_idlevomitblood;
			spr_crouchshoot = spr_peppino_crouchshoot;
			spr_facehurtup = spr_peppino_facehurtup;
			spr_facehurt = spr_peppino_facehurt;
			spr_breakdance = spr_peppino_breakdance;
			spr_shotgun_player = spr_peppino_shotgun;
			spr_swingding = spr_peppino_swingding;
			spr_swingdingend = spr_peppino_swingdingend;
			spr_downslopes = spr_peppino_downslopes;
			spr_upslopes = spr_peppino_upslopes;
			spr_ceilingcrash = spr_peppino_bump;
			spr_machfreefall = spr_peppino_machfreefall;
			spr_superjumpprep = spr_peppino_superjumpprep;
			spr_superjumppreplight = spr_peppino_superjumppreplight;
			spr_superjumpright = spr_peppino_superjumpright;
			spr_superjumpleft = spr_peppino_superjumpleft;
			spr_superjump = spr_peppino_superjump;
			spr_superjump_cancel = spr_peppino_Sjumpcancel;
			spr_superjump_cancelgroundedmach4 = spr_pizzano_superjump_cancelmach4grounded;
			spr_superjump_cancelgrounded = spr_pizzano_mach3;
			spr_superjump_cancelprep = spr_peppino_Sjumpcancelstart
			spr_superjumpland = spr_peppino_superjumpland;
			spr_suplexdashCancel = spr_peppino_suplexcancel;
			spr_machtumble = spr_peppino_faceplant;
			spr_facestomp = spr_peppino_facestomp;
			spr_freefall =  spr_peppino_freefall;
			spr_freefall2 = spr_peppino_freefall;
			spr_freefallland = spr_peppino_freefallland;
			spr_small_destroyable = spr_towerblocksmall;
			spr_large_destroyable = spr_towerblock;
			spr_large_destroyable_hard = spr_harddestroyable;
			spr_small_destroyable_collect = spr_destroyable2;
			spr_large_destroyable_collect = spr_bigbreakable;
			spr_metal_destroyable = spr_metaltowerblock;
			spr_small_destroyable_escape = spr_johndestructible2;
			spr_large_destroyable_escape = spr_johndestructible;
			spr_small_destroyable_collect_escape = spr_destroyable21;
			spr_large_destroyable_collect_escape = spr_bigbreakableescape;
			spr_metal_destroyable_escape = spr_metaltowerescape;
			spr_metal_destroyable_hard = spr_metalb;
			spr_smallcollect1 = spr_shroomcollect;
			spr_smallcollect2 = spr_cheesecollect;
			spr_smallcollect3 = spr_tomatocollect;
			spr_smallcollect4 = spr_sausagecollect;
			spr_smallcollect5 = spr_pineapplecollect;
			spr_bigcollect1 = spr_pizzacollect1;
			spr_bigcollect2 = spr_pizzacollect2;
			spr_bigcollect3 = spr_pizzacollect3;
			spr_bigcollect4 = spr_pizzacollect4;
			spr_bigcollect5 = spr_pizzacollect5;
			spr_massivecollect1 = spr_giantpizza;
			spr_massivecollect2 = spr_giantpizza;
			spr_massivecollect3 = spr_giantpizza;
			spr_massivecollect4 = spr_giantpizza;
			spr_smallcollectescape1 = spr_escapecollect;
			spr_bigcollectescape1 = spr_escapecollectbig;
			spr_collectslice1 = spr_pizzaslice;
			spr_confectcage = spr_pizzaboxunopen;
			spr_confectcage_hop = spr_pizzaboxunopenhop;
			spr_confectcage_help = spr_toppinhelp;
			spr_gummyharry_idle = spr_hungrypillar;
			spr_gummyharry_dead = spr_hungrypillar_dead;
			spr_player_rankE = spr_peppino_rankP;
			spr_player_rankP = spr_peppino_rankP;
			spr_player_rankS = spr_peppino_rankS;
			spr_player_rankA = spr_peppino_rankA;
			spr_player_rankB = spr_peppino_rankB;
			spr_player_rankC = spr_peppino_rankC;
			spr_player_rankD = spr_peppino_rankD;
			spr_rankbubble_P = spr_rankbubblepep_P;
			spr_rankbubble_Pfilled = spr_rankbubblepep_P;
			spr_rankbubble_S = spr_rankbubblepep_S;
			spr_rankbubble_Sfilled = spr_rankbubblepep_S;
			spr_rankbubble_A = spr_rankbubblepep_A;
			spr_rankbubble_Afilled = spr_rankbubblepep_Afill;
			spr_rankbubble_B = spr_rankbubblepep_B;
			spr_rankbubble_Bfilled = spr_rankbubblepep_Bfill;
			spr_rankbubble_C = spr_rankbubblepep_C;
			spr_rankbubble_Cfilled = spr_rankbubblepep_Cfill;
			spr_rankbubble_D = spr_rankbubblepep_D;
			spr_rankbubble_Dfilled = spr_rankbubblepep_Dfill;
			spr_confecti5_appear = spr_toppinpineapple_intro;
			spr_confecti5_idle = spr_toppinpineapple;
			spr_confecti5_walk = spr_toppinpineapple_run;
			spr_confecti5_panic = spr_toppinpineapple_panic;
			spr_confecti5_panicWalk = spr_toppinpineapple_run;
			spr_confecti5_taunt = spr_toppinpineapple_taunt;
			spr_confecti4_appear = spr_toppinsausage_intro;
			spr_confecti4_idle = spr_toppinsausage;
			spr_confecti4_walk = spr_toppinsausage_run;
			spr_confecti4_panic = spr_toppinsausage_panic;
			spr_confecti4_panicWalk = spr_toppinsausage_run;
			spr_confecti4_taunt = spr_toppinsausage_taunt;
			spr_confecti3_appear = spr_toppintomato_intro;
			spr_confecti3_idle = spr_toppintomato;
			spr_confecti3_walk = spr_toppintomato_run;
			spr_confecti3_panic = spr_toppintomato_panic;
			spr_confecti3_panicWalk = spr_toppintomato_run;
			spr_confecti3_taunt = spr_toppintomato_taunt;
			spr_confecti2_appear = spr_toppincheese_intro;
			spr_confecti2_idle = spr_toppincheese;
			spr_confecti2_walk = spr_toppincheese_run;
			spr_confecti2_panic = spr_toppincheese_panic;
			spr_confecti2_panicWalk = spr_toppincheese_run;
			spr_confecti2_taunt = spr_toppincheese_taunt;
			spr_confecti1_appear = spr_toppinshroom_intro;
			spr_confecti1_idle = spr_toppinshroom;
			spr_confecti1_walk = spr_toppinshroom_run;
			spr_confecti1_panic = spr_toppinshroom_panic;
			spr_confecti1_panicWalk = spr_toppinshroom_run;
			spr_confecti1_taunt = spr_toppinshroom_taunt;
			spr_janitor_appear = spr_gerome_collected;
			spr_janitor_idle = spr_gerome_keyidle;
			spr_janitor_walk = spr_gerome_keymove;
			spr_janitor_panic = spr_gerome_keyidle;
			spr_janitor_panicWalk = spr_gerome_keymove;
			spr_janitor_taunt = spr_gerome_taunt;
			spr_large_destroyable_oneway = spr_onewaybigblock;
			spr_fireass = spr_peppino_fireass;
			spr_bottlerocketstart = spr_peppino_rocketstart;
			spr_lapvisual_player = spr_lapvisual
			spr_lapbg_player = spr_lapbg
			spr_coneball_player = spr_pizzaface
			spr_coneball_melting_player = spr_pizzahead_haywire
			img_featuringme = 4;
			spr_stylebar = spr_heatmeter
			spr_stylebarunder = spr_heatmeter_fill
			spr_collecthud = spr_pizzascore
			spr_crankhud = spr_pizzascore_olive
			spr_brankhud = spr_pizzascore_pepper
			spr_arankhud = spr_pizzascore_shroom
			spr_srankhud = spr_pizzascore_pepperoni
			spr_gatehud = spr_pizzascore
			spr_combotitle = spr_comboend_title1
			spr_combotitle_BR = spr_comboend_title1
			spr_combotitle_ES = spr_comboend_title1
			spr_combotitle_truly = spr_combovery
			spr_roomnamehud = spr_roomnamebg
			spr_lapportal_create = spr_pizzaportal_exit
			spr_lapportal_idle = spr_pizzaportal
			spr_lapportal_enter = spr_pizzaportal_enter
			spr_lapportal_exit = spr_pizzaportal_exit
			spr_heatmeterpal = spr_heatmeter_palette
			spr_escapepopup = spr_itspizzatime
			// fonts
			font_lapcount = global.lapcountpepfont
			font_collect_new = global.collectpepfont
			font_collect = global.collectpepfont
			break;
		case "T":
			spr_idle = spr_noise_idle;
			spr_move = spr_noise_move;
			spr_angryidle = spr_noise_angryidle;
			spr_angrywalk = spr_noise_angrymove;
			spr_crawl = spr_noise_crawl;
			spr_hurt = spr_noise_hurt;
			spr_jump = spr_noise_jump;
			spr_jump2 = spr_noise_jump;
			spr_fall = spr_noise_fall;
			spr_fall2 = spr_noise_fall;
			spr_bodyslamstart = spr_noise_bodyslamstart;
			spr_bodyslamfall = spr_noise_bodyslam;
			spr_bodyslamland = spr_noise_bodyslamland;
			spr_bombdropstart = spr_noise_bodyslamstart;
			spr_bombdropfall = spr_noise_bodyslam;
			spr_bombdropland = spr_noise_bodyslamland;
			spr_crouch = spr_noise_crouch;
			spr_crouchjump = spr_noise_crouchjump;
			spr_crouchfall = spr_noise_crouchfall;
			spr_couchstart = spr_noise_crouchstart;
			spr_bump = spr_noise_bump;
			spr_suplexbump = spr_noise_suplexbump
			spr_land = spr_noise_land;
			spr_land2 = spr_noise_land2;
			spr_lookdoor = spr_noise_lookdoor;
			spr_walkfront = spr_noise_walkfront;
			spr_victory = spr_noise_entergate;
			spr_Ladder = spr_noise_ladder;
			spr_laddermove = spr_noise_laddermove;
			spr_ladderdown = spr_noise_ladderdown;
			spr_keyget = spr_noise_keyget;
			spr_crouchslip = spr_noise_crouchslip;
			spr_slippingonice = spr_noise_slipnslide;
			spr_mach1 = spr_noise_mach1;
			spr_mach2 = spr_noise_mach;
			spr_mach3player = spr_noise_mach3;
			spr_machdashpad = spr_noise_dashpad;
			spr_rollgetup = spr_noise_rollgetup;
			spr_mach3jump = spr_noise_mach3jump;
			spr_mach3hitwall = spr_noise_mach3hitwall;
			spr_crazyrun = spr_noise_crazyrun;
			spr_secondjump1 = spr_noise_secondjump1;
			spr_secondjump2 = spr_noise_secondjump2;
			spr_trick = spr_noise_secondjump1
			spr_machslidestart = spr_noise_machslidestart;
			spr_machslide = spr_noise_machslide;
			spr_machslideend = spr_noise_machslideend;
			spr_grindrail = spr_noise_grind;
			spr_machslideboost = spr_noise_machslideboost;
			spr_machslideboost3 = spr_noise_mach3boost;
			spr_superjump_cancelmach4 = spr_noise_jetpackboost
			spr_superjump_cancelgrounded = spr_noise_jetpackboost
			spr_superjump_cancelgroundedmach4 = spr_noise_jetpackboost
			spr_land2 = spr_noise_land2;
			spr_punch = spr_noise_punch;
			spr_backkick = spr_noise_backkick;
			spr_shoulder = spr_noise_shoulder;
			spr_uppunch = spr_noise_charge;
			spr_stomp = spr_noise_stomp;
			spr_stompprep = spr_noise_stompprep;
			spr_crouchslide = spr_noise_jetpackslide;;
			spr_crouch = spr_noise_crouch;
			spr_bump = spr_noise_bump;
			spr_victory = spr_noise_entergate;
			spr_hurt = spr_noise_hurt;
			spr_climbwall = spr_noise_climbwall;
			spr_grab = spr_noise_haulingidle;
			spr_mach2jump = spr_noise_mach2jump;
			spr_longjump = spr_noise_mach2jump;
			spr_longjumpend = spr_noise_mach2jump;
			spr_walljumpstart = spr_noise_walljumpstart;
			spr_walljumpend = spr_noise_walljumpend;
			spr_mach3rollstart = spr_noise_machroll;
			spr_mach3roll = spr_noise_machroll;
			spr_Timesup = spr_noise_timesup;
			spr_deathstart = spr_noise_deathstart;
			spr_deathend = spr_noise_deathend;
			spr_machpunch1 = spr_noise_machpunch1;
			spr_machpunch2 = spr_noise_machpunch2;
			spr_hurtjump = spr_noise_hurt;
			spr_entergate = spr_noise_entergate;
			spr_gottreasure = spr_noise_entergate;
			spr_bossintro = spr_noise_bossintro;
			spr_hurtidle = spr_noise_hurtidle;
			spr_hurtwalk = spr_noise_hurtwalk;
			spr_suplexmash1 = spr_noise_finishingblow1;
			spr_suplexmash2 = spr_noise_finishingblow2;
			spr_suplexmash3 = spr_noise_finishingblow3;
			spr_suplexmash4 = spr_noise_finishingblow4;
			  spr_blockbreak1 = spr_noise_suplexmash1;
			spr_blockbreak2 = spr_noise_suplexmash2;
			spr_blockbreak3 = spr_noise_suplexmash3;
			spr_blockbreak4 = spr_noise_suplexmash4;
			spr_blockbreak5 = spr_noise_punch;
			spr_blockbreak6 = spr_noise_shoulder;
			spr_blockbreak7 = spr_noise_suplexmash1;
			spr_tackle = spr_noise_tackle;
			spr_airdash1 = spr_noise_airdash1;
			spr_airdash2 = spr_noise_airdash2;
			spr_keyget = spr_noise_keyget;
			spr_idle1 = spr_noise_idlemask;
			spr_idle2 = spr_noise_idlesmoke;
			spr_idle3 = spr_noise_idlegun;
			spr_idle4 = spr_noise_idleface;
			spr_parry1 = spr_noise_parry1;
			spr_parry2 = spr_noise_parry2;
			spr_parry3 = spr_noise_parry3;
			spr_supertaunt1 = spr_noise_supertaunt1;
			spr_supertaunt2 = spr_noise_supertaunt2;
			spr_supertaunt3 = spr_noise_supertaunt3;
			spr_suplexdash = spr_noise_suplexgrab;
			spr_piledriver = spr_noise_piledriver;
			spr_piledriverland = spr_noise_piledriverland;
			spr_charge = spr_noise_charge;
			spr_taunt = spr_noise_taunt;
			spr_uppizzabox = spr_noise_uppizzabox;
			spr_downpizzabox = spr_noise_downpizzabox;
			spr_tumblestart = spr_noise_tumblestart;
			spr_tumble = spr_noise_tumble;
			spr_tumbleend = spr_noise_tumbleend;
			spr_machroll = spr_noise_machroll;
			spr_dive = spr_noise_dive;
			spr_cottonidle = spr_cotton_idle;
			spr_escapeidle = spr_noise_panicidle;
			spr_timesupidle = spr_noise_hurtidle;
			spr_haulingstart = spr_noise_haulingstart;
			spr_haulingidle = spr_noise_haulingidle;
			spr_haulingfall = spr_noise_haulingfall;
			spr_haulingjump = spr_noise_haulingjump;
			spr_haulingland = spr_noise_haulingland;
			spr_haulingwalk = spr_noise_haulingmove;
			spr_uppercutfinishingblow = spr_noise_uppercut;
			spr_breakdance = spr_noise_idledance
			spr_superjumpprep = spr_noise_Sjumpstart
			spr_superjumppreplight = spr_noise_Sjumpprep
			spr_superjumpright = spr_noise_Sjumpmoveright
			spr_superjumpleft = spr_noise_Sjumpmoveleft
			spr_superjump = spr_noise_Sjump
			spr_superjump_cancel = spr_noise_jetpackboost
			spr_superjump_cancelprep = spr_noise_jetpackstart
			spr_superjumpland = spr_noise_Sjumpland
			spr_suplexdashCancel = spr_noise_suplexcancel
			spr_outofcontrolfall = spr_noise_slipbanan1
			spr_winding = spr_noise_winding;
			spr_danceidle = spr_noise_idledance;
			spr_uppercutbegin = spr_noise_doublejump;
			spr_uppercutend = spr_noise_doublejumpfall;
			spr_uppercutfinishingblow = spr_noise_uppercut;
			spr_skatecrouch = spr_noise_clowncrouch;
			spr_facehurtup = spr_noise_facehurtup;
			spr_facehurt = spr_noise_facehurt;
			spr_shotgun_player = spr_peppino_shotgun;
			spr_swingding = spr_noise_swingding;
			spr_swingdingend = spr_noise_swingdingend;
			spr_machtumble = spr_noise_spin;
			spr_small_destroyable = spr_towerblocksmall;
			spr_large_destroyable = spr_towerblock;
			spr_large_destroyable_hard = spr_harddestroyable;
			spr_small_destroyable_collect = spr_halloweensmallblock;
			spr_large_destroyable_collect = spr_halloweenbigblock;
			spr_metal_destroyable = spr_metaltowerblock;
			spr_small_destroyable_escape = spr_johndestructible2;
			spr_large_destroyable_escape = spr_johndestructible;
			spr_small_destroyable_collect_escape = spr_halloweensmallblockescape;
			spr_large_destroyable_collect_escape = spr_halloweenbigblockescape;
			spr_metal_destroyable_escape = spr_metaltowerescape;
			spr_metal_destroyable_hard = spr_metalb;
			spr_smallcollect1 = spr_shroomcollect;
			spr_smallcollect2 = spr_cheesecollect;
			spr_smallcollect3 = spr_tomatocollect;
			spr_smallcollect4 = spr_sausagecollect;
			spr_smallcollect5 = spr_pineapplecollect;
			spr_bigcollect1 = spr_pizzacollect1;
			spr_bigcollect2 = spr_pizzacollect2;
			spr_bigcollect3 = spr_pizzacollect3;
			spr_bigcollect4 = spr_pizzacollect4;
			spr_bigcollect5 = spr_pizzacollect5;
			spr_massivecollect1 = spr_giantpizzahalloween;
			spr_massivecollect2 = spr_giantpizzahalloween;
			spr_massivecollect3 = spr_giantpizzahalloween;
			spr_massivecollect4 = spr_giantpizzahalloween;
			spr_smallcollectescape1 = spr_escapecollect;
			spr_bigcollectescape1 = spr_escapecollectbig;
			spr_collectslice1 = spr_halloweenspinningpizza;
			spr_confectcage = spr_pizzaboxunopen;
			spr_confectcage_hop = spr_pizzaboxunopenhop;
			spr_confectcage_help = spr_toppinhelp;
			spr_gummyharry_idle = spr_hungrypillar;
			spr_gummyharry_dead = spr_hungrypillar_dead;
			spr_player_rankE = spr_peppino_rankP;
			spr_player_rankP = spr_peppino_rankP;
			spr_player_rankS = spr_noise_rankS;
			spr_player_rankA = spr_noise_rankA;
			spr_player_rankB = spr_noise_rankB;
			spr_player_rankC = spr_noise_rankC;
			spr_player_rankD = spr_noise_rankD;
			spr_rankwait = spr_noise_rankwait;
			spr_rankbubble_P = spr_rankbubblepep_P;
			spr_rankbubble_Pfilled = spr_rankbubblepep_P;
			spr_rankbubble_S = spr_rankbubblepep_S;
			spr_rankbubble_Sfilled = spr_rankbubblepep_S;
			spr_rankbubble_A = spr_rankbubblepep_A;
			spr_rankbubble_Afilled = spr_rankbubblepep_Afill;
			spr_rankbubble_B = spr_rankbubblepep_B;
			spr_rankbubble_Bfilled = spr_rankbubblepep_Bfill;
			spr_rankbubble_C = spr_rankbubblepep_C;
			spr_rankbubble_Cfilled = spr_rankbubblepep_Cfill;
			spr_rankbubble_D = spr_rankbubblepep_D;
			spr_rankbubble_Dfilled = spr_rankbubblepep_Dfill;
			spr_confecti5_appear = spr_toppinpineapple_intro;
			spr_confecti5_idle = spr_toppinpineapple;
			spr_confecti5_walk = spr_toppinpineapple_run;
			spr_confecti5_panic = spr_toppinpineapple_panic;
			spr_confecti5_panicWalk = spr_toppinpineapple_run;
			spr_confecti5_taunt = spr_toppinpineapple_taunt;
			spr_confecti4_appear = spr_toppinsausage_intro;
			spr_confecti4_idle = spr_toppinsausage;
			spr_confecti4_walk = spr_toppinsausage_run;
			spr_confecti4_panic = spr_toppinsausage_panic;
			spr_confecti4_panicWalk = spr_toppinsausage_run;
			spr_confecti4_taunt = spr_toppinsausage_taunt;
			spr_confecti3_appear = spr_toppintomato_intro;
			spr_confecti3_idle = spr_toppintomato;
			spr_confecti3_walk = spr_toppintomato_run;
			spr_confecti3_panic = spr_toppintomato_panic;
			spr_confecti3_panicWalk = spr_toppintomato_run;
			spr_confecti3_taunt = spr_toppintomato_taunt;
			spr_confecti2_appear = spr_toppincheese_intro;
			spr_confecti2_idle = spr_toppincheese;
			spr_confecti2_walk = spr_toppincheese_run;
			spr_confecti2_panic = spr_toppincheese_panic;
			spr_confecti2_panicWalk = spr_toppincheese_run;
			spr_confecti2_taunt = spr_toppincheese_taunt;
			spr_confecti1_appear = spr_toppinshroom_intro;
			spr_confecti1_idle = spr_toppinshroom;
			spr_confecti1_walk = spr_toppinshroom_run;
			spr_confecti1_panic = spr_toppinshroom_panic;
			spr_confecti1_panicWalk = spr_toppinshroom_run;
			spr_confecti1_taunt = spr_toppinshroom_taunt;
			spr_janitor_appear = spr_gerome_collected;
			spr_janitor_idle = spr_gerome_keyidle;
			spr_janitor_walk = spr_gerome_keymove;
			spr_janitor_panic = spr_gerome_keyidle;
			spr_janitor_panicWalk = spr_gerome_keymove;
			spr_janitor_taunt = spr_gerome_taunt;
			spr_large_destroyable_oneway = spr_onewaybigblock;
			spr_lapvisual_player = spr_lapvisual_pep
			spr_lapbg_player = spr_lapbg_pep
			spr_coneball_player = spr_pizzaface
			spr_coneball_melting_player = spr_pizzahead_haywire
			img_featuringme = 5;
			spr_stylebar = spr_heatmeter_pep
			spr_stylebarunder = spr_heatmeter_fill
			spr_collecthud = spr_pizzascore
			spr_crankhud = spr_pizzascore_olive
			spr_brankhud = spr_pizzascore_pepper
			spr_arankhud = spr_pizzascore_shroom
			spr_srankhud = spr_pizzascore_pepperoni
			spr_gatehud = spr_pizzascore
			spr_combotitle = spr_comboend_title1
			spr_combotitle_BR = spr_comboend_title1
			spr_combotitle_ES = spr_comboend_title1
			spr_combotitle_truly = spr_combovery
			spr_roomnamehud = spr_roomnamebg_pep
			spr_lapportal_create = spr_pizzaportal_exit
			spr_lapportal_idle = spr_pizzaportal
			spr_lapportal_enter = spr_pizzaportal_enter
			spr_lapportal_exit = spr_pizzaportal_exit
			spr_heatmeterpal = spr_heatmeter_palette
			spr_escapepopup = spr_itspizzatime
			// fonts
			font_lapcount = global.lapcountpepfont
			font_collect_new = global.collectpepfont
			font_collect = global.collectpepfont
			// audio
			mu_escapesong = mu_noise_escape
			mu_lap2song = mu_noise_lap
			mu_lap5song = mu_peppino_despairy
			mu_lap10song = mu_peppino_death
			sfx_lapbell = sfx_lapbells_pep
			sfx_escapecollectbig = sfx_bellcollectbig
			sfx_escapecollect = sfx_bellcollectsmall
			sfx_collectmassive = sfx_collectgiant
			sfx_collectlarge = sfx_collectbig
			sfx_collect = sfx_collectsmall
			sfx_taunt = "sfx_tauntnoise"
			taunt_upperrange = 1
			sfx_supertaunt = sound_supertaunt1
			break;
		case "PT":
			spr_idle = spr_noise_idle;
			spr_move = spr_noise_move;
			spr_angryidle = spr_noise_angryidle;
			spr_angrywalk = spr_noise_angrymove;
			spr_crawl = spr_noise_crawl;
			spr_hurt = spr_noise_hurt;
			spr_jump = spr_noise_jump;
			spr_jump2 = spr_noise_jump;
			spr_fall = spr_noise_fall;
			spr_fall2 = spr_noise_fall;
			spr_bodyslamstart = spr_noise_bodyslamstart;
			spr_bodyslamfall = spr_noise_bodyslam;
			spr_bodyslamland = spr_noise_bodyslamland;
			spr_bombdropstart = spr_noise_bodyslamstart;
			spr_bombdropfall = spr_noise_bodyslam;
			spr_bombdropland = spr_noise_bodyslamland;
			spr_crouch = spr_noise_crouch;
			spr_crouchjump = spr_noise_crouchjump;
			spr_crouchfall = spr_noise_crouchfall;
			spr_couchstart = spr_noise_crouchstart;
			spr_bump = spr_noise_bump;
			spr_suplexbump = spr_noise_suplexbump
			spr_land = spr_noise_land;
			spr_land2 = spr_noise_land2;
			spr_lookdoor = spr_noise_lookdoor;
			spr_walkfront = spr_noise_walkfront;
			spr_victory = spr_noise_entergate;
			spr_Ladder = spr_noise_ladder;
			spr_laddermove = spr_noise_laddermove;
			spr_ladderdown = spr_noise_ladderdown;
			spr_keyget = spr_noise_keyget;
			spr_crouchslip = spr_noise_crouchslip;
			spr_slippingonice = spr_noise_slipnslide;
			spr_mach1 = spr_noise_mach1;
			spr_mach2 = spr_noise_mach;
			spr_mach3player = spr_noise_mach3;
			spr_machdashpad = spr_noise_dashpad;
			spr_rollgetup = spr_noise_rollgetup;
			spr_mach3jump = spr_noise_mach3jump;
			spr_mach3hitwall = spr_noise_mach3hitwall;
			spr_crazyrun = spr_noise_crazyrun;
			spr_secondjump1 = spr_noise_secondjump1;
			spr_secondjump2 = spr_noise_secondjump2;
			spr_trick = spr_noise_secondjump1
			spr_djump = spr_noise_doublejump
			spr_machslidestart = spr_noise_machslidestart;
			spr_machslide = spr_noise_machslide;
			spr_machslideend = spr_noise_machslideend;
			spr_grindrail = spr_noise_grind;
			spr_machslideboost = spr_noise_machslideboost;
			spr_machslideboost3 = spr_noise_mach3boost;
			spr_superjump_cancelmach4 = spr_noise_jetpackboost
			spr_superjump_cancelgrounded = spr_noise_jetpackboost
			spr_superjump_cancelgroundedmach4 = spr_noise_jetpackboost
			spr_land2 = spr_noise_land2;
			spr_punch = spr_noise_punch;
			spr_backkick = spr_noise_backkick;
			spr_shoulder = spr_noise_shoulder;
			spr_uppunch = spr_noise_charge;
			spr_stomp = spr_noise_stomp;
			spr_stompprep = spr_noise_stompprep;
			spr_crouchslide = spr_noise_jetpackslide;
			spr_crouch = spr_noise_crouch;
			spr_bump = spr_noise_bump;
			spr_victory = spr_noise_entergate;
			spr_hurt = spr_noise_hurt;
			spr_climbwall = spr_noise_climbwall;
			spr_grab = spr_noise_haulingidle;
			spr_mach2jump = spr_noise_mach2jump;
			spr_longjump = spr_noise_mach2jump;
			spr_longjumpend = spr_noise_mach2jump;
			spr_walljumpstart = spr_noise_walljumpstart;
			spr_walljumpend = spr_noise_walljumpend;
			spr_mach3rollstart = spr_noise_machroll;
			spr_mach3roll = spr_noise_machroll;
			spr_Timesup = spr_noise_timesup;
			spr_deathstart = spr_noise_deathstart;
			spr_deathend = spr_noise_deathend;
			spr_machpunch1 = spr_noise_machpunch1;
			spr_machpunch2 = spr_noise_machpunch2;
			spr_hurtjump = spr_noise_hurt;
			spr_entergate = spr_noise_entergate;
			spr_gottreasure = spr_noise_entergate;
			spr_bossintro = spr_noise_bossintro;
			spr_hurtidle = spr_noise_hurtidle;
			spr_hurtwalk = spr_noise_hurtwalk;
			spr_suplexmash1 = spr_noise_finishingblow1;
			spr_suplexmash2 = spr_noise_finishingblow2;
			spr_suplexmash3 = spr_noise_finishingblow3;
			spr_suplexmash4 = spr_noise_finishingblow4;
			spr_blockbreak1 = spr_noise_suplexmash1;
			spr_blockbreak2 = spr_noise_suplexmash2;
			spr_blockbreak3 = spr_noise_suplexmash3;
			spr_blockbreak4 = spr_noise_suplexmash4;
			spr_blockbreak5 = spr_noise_punch;
			spr_blockbreak6 = spr_noise_shoulder;
			spr_blockbreak7 = spr_noise_suplexmash1;
			spr_tackle = spr_noise_tackle;
			spr_airdash1 = spr_noise_airdash1;
			spr_airdash2 = spr_noise_airdash2;
			spr_keyget = spr_noise_keyget;
			spr_idle1 = spr_noise_idlemask;
			spr_idle2 = spr_noise_idlesmoke;
			spr_idle3 = spr_noise_idlegun;
			spr_idle4 = spr_noise_idleface;
			spr_parry1 = spr_noise_parry1;
			spr_parry2 = spr_noise_parry2;
			spr_parry3 = spr_noise_parry3;
			spr_supertaunt1 = spr_noise_supertaunt1;
			spr_supertaunt2 = spr_noise_supertaunt2;
			spr_supertaunt3 = spr_noise_supertaunt3;
			spr_suplexdash = spr_noise_suplexgrab;
			spr_piledriver = spr_noise_piledriver;
			spr_piledriverland = spr_noise_piledriverland;
			spr_charge = spr_noise_charge;
			spr_taunt = spr_noise_taunt;
			spr_uppizzabox = spr_noise_uppizzabox;
			spr_downpizzabox = spr_noise_downpizzabox;
			spr_tumblestart = spr_noise_tumblestart;
			spr_tumble = spr_noise_tumble;
			spr_tumbleend = spr_noise_tumbleend;
			spr_machroll = spr_noise_machroll;
			spr_dive = spr_noise_dive;
			spr_cottonidle = spr_cotton_idle;
			spr_escapeidle = spr_noise_panicidle;
			spr_timesupidle = spr_noise_hurtidle;
			spr_haulingstart = spr_noise_haulingstart;
			spr_haulingidle = spr_noise_haulingidle;
			spr_haulingfall = spr_noise_haulingfall;
			spr_haulingjump = spr_noise_haulingjump;
			spr_haulingland = spr_noise_haulingland;
			spr_haulingwalk = spr_noise_haulingmove;
			spr_uppercutfinishingblow = spr_noise_uppercut;
			spr_breakdance = spr_noise_idledance
			spr_superjumpprep = spr_noise_Sjumpstart
			spr_superjumppreplight = spr_noise_Sjumpprep
			spr_superjumpright = spr_noise_Sjumpmoveright
			spr_superjumpleft = spr_noise_Sjumpmoveleft
			spr_superjump = spr_noise_Sjump
			spr_superjump_cancel = spr_noise_jetpackboost
			spr_superjump_cancelprep = spr_noise_jetpackstart
			spr_superjumpland = spr_noise_Sjumpland
			spr_suplexdashCancel = spr_noise_suplexcancel
			spr_outofcontrolfall = spr_noise_slipbanan1
			spr_winding = spr_noise_winding;
			spr_danceidle = spr_noise_idledance;
			spr_uppercutbegin = spr_noise_doublejump;
			spr_uppercutend = spr_noise_doublejumpfall;
			spr_uppercutfinishingblow = spr_noise_uppercut;
			spr_skatecrouch = spr_noise_clowncrouch;
			spr_facehurtup = spr_noise_facehurtup;
			spr_facehurt = spr_noise_facehurt;
			spr_shotgun_player = spr_peppino_shotgun;
			spr_swingding = spr_noise_swingding;
			spr_swingdingend = spr_noise_swingdingend;
			spr_machtumble = spr_noise_spin;
			spr_small_destroyable = spr_towerblocksmall;
			spr_large_destroyable = spr_towerblock;
			spr_large_destroyable_hard = spr_harddestroyable;
			spr_small_destroyable_collect = spr_halloweensmallblock;
			spr_large_destroyable_collect = spr_halloweenbigblock;
			spr_metal_destroyable = spr_metaltowerblock;
			spr_small_destroyable_escape = spr_johndestructible2;
			spr_large_destroyable_escape = spr_johndestructible;
			spr_small_destroyable_collect_escape = spr_halloweensmallblockescape;
			spr_large_destroyable_collect_escape = spr_halloweenbigblockescape;
			spr_metal_destroyable_escape = spr_metaltowerescape;
			spr_metal_destroyable_hard = spr_metalb;
			spr_smallcollect1 = spr_shroomcollect;
			spr_smallcollect2 = spr_cheesecollect;
			spr_smallcollect3 = spr_tomatocollect;
			spr_smallcollect4 = spr_sausagecollect;
			spr_smallcollect5 = spr_pineapplecollect;
			spr_bigcollect1 = spr_pizzacollect1;
			spr_bigcollect2 = spr_pizzacollect2;
			spr_bigcollect3 = spr_pizzacollect3;
			spr_bigcollect4 = spr_pizzacollect4;
			spr_bigcollect5 = spr_pizzacollect5;
			spr_massivecollect1 = spr_giantpizzahalloween;
			spr_massivecollect2 = spr_giantpizzahalloween;
			spr_massivecollect3 = spr_giantpizzahalloween;
			spr_massivecollect4 = spr_giantpizzahalloween;
			spr_smallcollectescape1 = spr_escapecollect;
			spr_bigcollectescape1 = spr_escapecollectbig;
			spr_collectslice1 = spr_halloweenspinningpizza;
			spr_confectcage = spr_pizzaboxunopen;
			spr_confectcage_hop = spr_pizzaboxunopenhop;
			spr_confectcage_help = spr_toppinhelp;
			spr_gummyharry_idle = spr_hungrypillar;
			spr_gummyharry_dead = spr_hungrypillar_dead;
			spr_player_rankE = spr_peppino_rankP;
			spr_player_rankP = spr_peppino_rankP;
			spr_player_rankS = spr_noise_rankS;
			spr_player_rankA = spr_noise_rankA;
			spr_player_rankB = spr_noise_rankB;
			spr_player_rankC = spr_noise_rankC;
			spr_player_rankD = spr_noise_rankD;
			spr_rankwait = spr_noise_rankwait;
			spr_rankbubble_P = spr_rankbubblepep_P;
			spr_rankbubble_Pfilled = spr_rankbubblepep_P;
			spr_rankbubble_S = spr_rankbubblepep_S;
			spr_rankbubble_Sfilled = spr_rankbubblepep_S;
			spr_rankbubble_A = spr_rankbubblepep_A;
			spr_rankbubble_Afilled = spr_rankbubblepep_Afill;
			spr_rankbubble_B = spr_rankbubblepep_B;
			spr_rankbubble_Bfilled = spr_rankbubblepep_Bfill;
			spr_rankbubble_C = spr_rankbubblepep_C;
			spr_rankbubble_Cfilled = spr_rankbubblepep_Cfill;
			spr_rankbubble_D = spr_rankbubblepep_D;
			spr_rankbubble_Dfilled = spr_rankbubblepep_Dfill;
			spr_confecti5_appear = spr_toppinpineapple_intro;
			spr_confecti5_idle = spr_toppinpineapple;
			spr_confecti5_walk = spr_toppinpineapple_run;
			spr_confecti5_panic = spr_toppinpineapple_panic;
			spr_confecti5_panicWalk = spr_toppinpineapple_run;
			spr_confecti5_taunt = spr_toppinpineapple_taunt;
			spr_confecti4_appear = spr_toppinsausage_intro;
			spr_confecti4_idle = spr_toppinsausage;
			spr_confecti4_walk = spr_toppinsausage_run;
			spr_confecti4_panic = spr_toppinsausage_panic;
			spr_confecti4_panicWalk = spr_toppinsausage_run;
			spr_confecti4_taunt = spr_toppinsausage_taunt;
			spr_confecti3_appear = spr_toppintomato_intro;
			spr_confecti3_idle = spr_toppintomato;
			spr_confecti3_walk = spr_toppintomato_run;
			spr_confecti3_panic = spr_toppintomato_panic;
			spr_confecti3_panicWalk = spr_toppintomato_run;
			spr_confecti3_taunt = spr_toppintomato_taunt;
			spr_confecti2_appear = spr_toppincheese_intro;
			spr_confecti2_idle = spr_toppincheese;
			spr_confecti2_walk = spr_toppincheese_run;
			spr_confecti2_panic = spr_toppincheese_panic;
			spr_confecti2_panicWalk = spr_toppincheese_run;
			spr_confecti2_taunt = spr_toppincheese_taunt;
			spr_confecti1_appear = spr_toppinshroom_intro;
			spr_confecti1_idle = spr_toppinshroom;
			spr_confecti1_walk = spr_toppinshroom_run;
			spr_confecti1_panic = spr_toppinshroom_panic;
			spr_confecti1_panicWalk = spr_toppinshroom_run;
			spr_confecti1_taunt = spr_toppinshroom_taunt;
			spr_janitor_appear = spr_gerome_collected;
			spr_janitor_idle = spr_gerome_keyidle;
			spr_janitor_walk = spr_gerome_keymove;
			spr_janitor_panic = spr_gerome_keyidle;
			spr_janitor_panicWalk = spr_gerome_keymove;
			spr_janitor_taunt = spr_gerome_taunt;
			spr_large_destroyable_oneway = spr_onewaybigblock;
			spr_lapvisual_player = spr_lapvisual_pep
			spr_lapbg_player = spr_lapbg_pep
			spr_coneball_player = spr_pizzaface
			spr_coneball_melting_player = spr_pizzahead_haywire
			img_featuringme = 5;
			spr_stylebar = spr_heatmeter_pep
			spr_stylebarunder = spr_heatmeter_fill
			spr_collecthud = spr_pizzascore
			spr_crankhud = spr_pizzascore_olive
			spr_brankhud = spr_pizzascore_pepper
			spr_arankhud = spr_pizzascore_shroom
			spr_srankhud = spr_pizzascore_pepperoni
			spr_gatehud = spr_pizzascore
			spr_combotitle = spr_comboend_title1
			spr_combotitle_BR = spr_comboend_title1
			spr_combotitle_ES = spr_comboend_title1
			spr_combotitle_truly = spr_combovery
			spr_roomnamehud = spr_roomnamebg_pep
			spr_lapportal_create = spr_pizzaportal_exit
			spr_lapportal_idle = spr_pizzaportal
			spr_lapportal_enter = spr_pizzaportal_enter
			spr_lapportal_exit = spr_pizzaportal_exit
			spr_heatmeterpal = spr_heatmeter_palette
			spr_escapepopup = spr_itspizzatime
			// fonts
			font_lapcount = global.lapcountpepfont
			font_collect_new = global.collectpepfont
			font_collect = global.collectpepfont
			// audio
			mu_escapesong = mu_noise_escape
			mu_lap2song = mu_noise_lap
			mu_lap5song = mu_peppino_despairy
			mu_lap10song = mu_peppino_death
			sfx_lapbell = sfx_lapbells_pep
			sfx_escapecollectbig = sfx_bellcollectbig
			sfx_escapecollect = sfx_bellcollectsmall
			sfx_collectmassive = sfx_collectgiant
			sfx_collectlarge = sfx_collectbig
			sfx_collect = sfx_collectsmall
			sfx_taunt = "sfx_tauntnoise"
			taunt_upperrange = 1
			sfx_supertaunt = sound_supertaunt1
			break;
		case "V":
			spr_idle = spr_vigilante_idle;
			spr_move = spr_vigilante_move;
			spr_angryidle = spr_vigilante_angryidle;
			spr_angrywalk = spr_vigilante_angrymove;
			spr_crawl = spr_vigilante_crouchmove;
			spr_hurt = spr_vigilante_hurt;
			spr_jump = spr_vigilante_jump;
			spr_jump2 = spr_vigilante_jump;
			spr_fall = spr_vigilante_fall;
			spr_fall2 = spr_vigilante_fall;
			spr_bodyslamstart = spr_vigilante_bodyslamprep;
			spr_bodyslamfall = spr_vigilante_bodyslam;
			spr_bodyslamland = spr_vigilante_bodyslamland;
			spr_bombdropstart = spr_vigilante_bodyslamprep;
			spr_bombdropfall = spr_vigilante_bodyslam;
			spr_bombdropland = spr_vigilante_bodyslamland;
			spr_crouch = spr_vigilante_crouch;
			spr_crouchjump = spr_vigilante_crouchjumpstart;
			spr_crouchfall = spr_vigilante_crouchjump;
			spr_couchstart = spr_vigilante_crouchjump;
			spr_splat = spr_peppino_wallsplat
			spr_bump = spr_vigilante_bump;
			spr_laddermove = spr_peppino_laddermove
			spr_keyget = spr_vigilante_gotkey;
			spr_crouchslip = spr_vigilante_crouchmove;
			spr_slippingonice = spr_vigilante_slipping;
			spr_mach1 = spr_vigilante_mach1;
			spr_mach2 = spr_vigilante_bootsmove;
			spr_mach3player = spr_vigilante_mach3
			spr_rollgetup = spr_vigilante_tumbleend;
			spr_mach3jump = spr_vigilante_mach3jump;
			spr_mach3hitwall = spr_vigilante_hitwall;
			spr_crazyrun = spr_vigilante_crazyrun;
			spr_secondjump1 = spr_vigilante_bootsjump;
			spr_secondjump2 = spr_vigilante_bootsfall;
			spr_machslidestart = spr_vigilante_machslidestart;
			spr_machslide = spr_vigilante_machslide;
			spr_machslideend = spr_vigilante_machslideend;
			spr_grindrail = spr_vigilante_tumble;
			spr_machslideboost = spr_vigilante_mach2boost
			spr_machslideboost3 = spr_vigilante_mach3boost;
			spr_land2 = spr_vigilante_land;
			spr_stomp = spr_vigilante_stomp;
			spr_stompprep = spr_vigilante_stompprep;
			spr_crouchslide = spr_vigilante_crouch;
			spr_crouch = spr_vigilante_crouch;
			spr_bump = spr_vigilante_bump;
			spr_victory = spr_vigilante_gottreasure;
			spr_hurt = spr_vigilante_hurt;
			spr_climbwall = spr_vigilante_wallclimb;
			spr_mach2jump = spr_vigilante_spinjump2;
			spr_Timesup = spr_peppino_timesup
			spr_entergate = spr_vigilante_gate;
			spr_gottreasure = spr_vigilante_gottreasure;
			spr_suplexmash1 = spr_peppino_finishingblow1
			spr_suplexmash2 = spr_peppino_finishingblow2
			spr_suplexmash3 = spr_peppino_finishingblow3
			spr_suplexmash4 = spr_peppino_finishingblow4
			spr_idle1 = spr_vigilante_idle1;
			spr_idle2 = spr_vigilante_idle2;
			spr_idle3 = spr_vigilante_idle3;
			spr_idle4 = spr_vigilante_idle4;
			spr_idle5 = spr_vigilante_idle5;
			spr_idle6 = spr_vigilante_idle6;
			spr_uppizzabox = spr_vigilante_upbox;
			spr_downpizzabox = spr_vigilante_downbox;
			spr_tumblestart = spr_vigilante_tumblestart;
			spr_tumble = spr_vigilante_tumble;
			spr_tumbleend = spr_vigilante_tumbleend;
			spr_taunt = spr_vigilante_taunt;
			spr_machroll =  spr_vigilante_tumble;
			spr_dive = spr_vigilante_dive;
			spr_cottonidle = spr_cotton_idle;
			spr_escapeidle = spr_vigilante_panic;
			spr_timesupidle = spr_vigilante_panic;
			spr_uppercutbegin = spr_peppino_breakdanceuppercut
			spr_uppercutend = spr_peppino_breakdanceuppercutend
			spr_shotgun_player = spr_peppino_shotgun;
			spr_superjumpprep = spr_vigilante_facehurt;
			spr_superjumppreplight = spr_vigilante_facehurt;
			spr_superjumpright = spr_vigilante_facehurt;
			spr_superjumpleft = spr_vigilante_facehurt;
			spr_superjump = spr_vigilante_sjump;
			spr_superjump_cancel = spr_peppino_Sjumpcancel;
			spr_superjump_cancelgroundedmach4 = spr_pizzano_superjump_cancelmach4grounded;
			spr_superjump_cancelgrounded = spr_pizzano_mach3;
			spr_superjump_cancelprep = spr_peppino_Sjumpcancelstart;
			spr_suplexdashCancel = spr_peppino_suplexcancel
			spr_machtumble = spr_vigilante_divekickstart;
			spr_facestomp = spr_vigilante_freefall;
			spr_freefall =  spr_vigilante_bigfall1;
			spr_freefall2 = spr_vigilante_bigfall2;
			spr_small_destroyable = spr_towerblocksmall;
			spr_large_destroyable = spr_towerblock;
			spr_large_destroyable_hard = spr_harddestroyable;
			spr_small_destroyable_collect = spr_destroyable2;
			spr_large_destroyable_collect = spr_bigbreakable;
			spr_metal_destroyable = spr_metaltowerblock;
			spr_small_destroyable_escape = spr_johndestructible2;
			spr_large_destroyable_escape = spr_johndestructible;
			spr_small_destroyable_collect_escape = spr_destroyable21;
			spr_large_destroyable_collect_escape = spr_bigbreakableescape;
			spr_metal_destroyable_escape = spr_metaltowerescape;
			spr_metal_destroyable_hard = spr_metalb;
			spr_smallcollect1 = spr_shroomcollect;
			spr_smallcollect2 = spr_cheesecollect;
			spr_smallcollect3 = spr_tomatocollect;
			spr_smallcollect4 = spr_sausagecollect;
			spr_smallcollect5 = spr_pineapplecollect;
			spr_bigcollect1 = spr_pizzacollect1;
			spr_bigcollect2 = spr_pizzacollect2;
			spr_bigcollect3 = spr_pizzacollect3;
			spr_bigcollect4 = spr_pizzacollect4;
			spr_bigcollect5 = spr_pizzacollect5;
			spr_massivecollect1 = spr_giantpizza;
			spr_massivecollect2 = spr_giantpizza;
			spr_massivecollect3 = spr_giantpizza;
			spr_massivecollect4 = spr_giantpizza;
			spr_smallcollectescape1 = spr_escapecollect;
			spr_bigcollectescape1 = spr_escapecollectbig;
			spr_collectslice1 = spr_pizzaslice;
			spr_confectcage = spr_pizzaboxunopen;
			spr_confectcage_hop = spr_pizzaboxunopenhop;
			spr_confectcage_help = spr_toppinhelp;
			spr_gummyharry_idle = spr_hungrypillar;
			spr_gummyharry_dead = spr_hungrypillar_dead;
			spr_player_rankE = spr_peppino_rankP;
			spr_player_rankP = spr_peppino_rankP;
			spr_player_rankS = spr_noise_rankS;
			spr_player_rankA = spr_noise_rankA;
			spr_player_rankB = spr_noise_rankB;
			spr_player_rankC = spr_noise_rankC;
			spr_player_rankD = spr_noise_rankD;
			spr_rankwait = spr_noise_rankwait;
			spr_rankbubble_P = spr_rankbubblepep_P;
			spr_rankbubble_Pfilled = spr_rankbubblepep_P;
			spr_rankbubble_S = spr_rankbubblepep_S;
			spr_rankbubble_Sfilled = spr_rankbubblepep_S;
			spr_rankbubble_A = spr_rankbubblepep_A;
			spr_rankbubble_Afilled = spr_rankbubblepep_Afill;
			spr_rankbubble_B = spr_rankbubblepep_B;
			spr_rankbubble_Bfilled = spr_rankbubblepep_Bfill;
			spr_rankbubble_C = spr_rankbubblepep_C;
			spr_rankbubble_Cfilled = spr_rankbubblepep_Cfill;
			spr_rankbubble_D = spr_rankbubblepep_D;
			spr_rankbubble_Dfilled = spr_rankbubblepep_Dfill;
			spr_confecti5_appear = spr_toppinpineapple_intro;
			spr_confecti5_idle = spr_toppinpineapple;
			spr_confecti5_walk = spr_toppinpineapple_run;
			spr_confecti5_panic = spr_toppinpineapple_panic;
			spr_confecti5_panicWalk = spr_toppinpineapple_run;
			spr_confecti5_taunt = spr_toppinpineapple_taunt;
			spr_confecti4_appear = spr_toppinsausage_intro;
			spr_confecti4_idle = spr_toppinsausage;
			spr_confecti4_walk = spr_toppinsausage_run;
			spr_confecti4_panic = spr_toppinsausage_panic;
			spr_confecti4_panicWalk = spr_toppinsausage_run;
			spr_confecti4_taunt = spr_toppinsausage_taunt;
			spr_confecti3_appear = spr_toppintomato_intro;
			spr_confecti3_idle = spr_toppintomato;
			spr_confecti3_walk = spr_toppintomato_run;
			spr_confecti3_panic = spr_toppintomato_panic;
			spr_confecti3_panicWalk = spr_toppintomato_run;
			spr_confecti3_taunt = spr_toppintomato_taunt;
			spr_confecti2_appear = spr_toppincheese_intro;
			spr_confecti2_idle = spr_toppincheese;
			spr_confecti2_walk = spr_toppincheese_run;
			spr_confecti2_panic = spr_toppincheese_panic;
			spr_confecti2_panicWalk = spr_toppincheese_run;
			spr_confecti2_taunt = spr_toppincheese_taunt;
			spr_confecti1_appear = spr_toppinshroom_intro;
			spr_confecti1_idle = spr_toppinshroom;
			spr_confecti1_walk = spr_toppinshroom_run;
			spr_confecti1_panic = spr_toppinshroom_panic;
			spr_confecti1_panicWalk = spr_toppinshroom_run;
			spr_confecti1_taunt = spr_toppinshroom_taunt;
			spr_janitor_appear = spr_gerome_collected;
			spr_janitor_idle = spr_gerome_keyidle;
			spr_janitor_walk = spr_gerome_keymove;
			spr_janitor_panic = spr_gerome_keyidle;
			spr_janitor_panicWalk = spr_gerome_keymove;
			spr_janitor_taunt = spr_gerome_taunt;
			spr_large_destroyable_oneway = spr_onewaybigblock;
			spr_bottlerocketstart = spr_peppino_rocketstart;
			spr_lapvisual_player = spr_lapvisual_pep
			spr_lapbg_player = spr_lapbg_pep
			spr_coneball_player = spr_pizzaface
			spr_coneball_melting_player = spr_pizzahead_haywire
			img_featuringme = 6;
			spr_stylebar = spr_heatmeter_pep
			spr_stylebarunder = spr_heatmeter_fill
			spr_collecthud = spr_pizzascore
			spr_crankhud = spr_pizzascore_olive
			spr_brankhud = spr_pizzascore_pepper
			spr_arankhud = spr_pizzascore_shroom
			spr_srankhud = spr_pizzascore_pepperoni
			spr_gatehud = spr_pizzascore
			spr_combotitle = spr_comboend_title1
			spr_combotitle_BR = spr_comboend_title1
			spr_combotitle_ES = spr_comboend_title1
			spr_combotitle_truly = spr_combovery
			spr_roomnamehud = spr_roomnamebg_pep
			spr_lapportal_create = spr_pizzaportal_exit
			spr_lapportal_idle = spr_pizzaportal
			spr_lapportal_enter = spr_pizzaportal_enter
			spr_lapportal_exit = spr_pizzaportal_exit
			spr_heatmeterpal = spr_heatmeter_palette
			spr_escapepopup = spr_itspizzatime
			// fonts
			font_lapcount = global.lapcountpepfont
			font_collect_new = global.collectpepfont
			font_collect = global.collectpepfont
			// audio
			mu_escapesong = mu_vigilante_escape
			mu_lap2song = mu_noise_lap
			mu_lap5song = mu_peppino_despairy
			mu_lap10song = mu_peppino_death
			sfx_lapbell = sfx_lapbells_pep
			sfx_escapecollectbig = sfx_bellcollectbig
			sfx_escapecollect = sfx_bellcollectsmall
			sfx_collectmassive = sfx_collectgiant
			sfx_collectlarge = sfx_collectbig
			sfx_collect = sfx_collectsmall
			sfx_taunt = "sfx_tauntvigilante"
			taunt_upperrange = 1
			sfx_supertaunt = sound_supertaunt1
			break;
		case "M":
			spr_idle = spr_pepperman_idle
			spr_move = spr_pepperman_move
			spr_jump = spr_pepperman_jump
			spr_bodyslamstart = spr_pepperman_groundpoundstart
			spr_bodyslamfall = spr_pepperman_groundpound
			spr_bodyslamland = spr_pepperman_groundpoundland
			spr_bombdropstart = spr_pepperman_groundpoundstart
			spr_bombdropfall = spr_pepperman_groundpound
			spr_bombdropland = spr_pepperman_groundpoundland
			spr_fall = spr_pepperman_fall
			spr_hurt = spr_pepperman_hurtplayer
			spr_entergate = spr_pepperman_lookdoor
			spr_mach3player = spr_pepperman_shoulderloop
			spr_machslideboost3 = spr_pepperman_shoulderturn
			spr_taunt = spr_pepperman_taunt
			spr_Timesup = spr_pepperman_timesup
			spr_walkfront = spr_pepperman_walkfront
			sfx_taunt = "sfx_tauntpepperman"
			taunt_upperrange = 1
			break;
		case "Z":
			// sprites
			spr_idle = spr_pizzall_idle;
			spr_move = spr_pizzall_walk;
			spr_angryidle = spr_pizzall_3hpidle;
			spr_angrywalk = spr_pizzall_3hpwalk;
			spr_crawl = spr_pizzall_machsliding;
			spr_hurt = spr_pizzall_hurt;
			spr_jump = spr_pizzall_jump;
			spr_jump2 = spr_pizzall_jump;
			spr_fall = spr_pizzall_fall;
			spr_fall2 = spr_pizzall_fall;
			spr_bodyslamstart = spr_pizzall_bodyslamstart;
			spr_bodyslamfall = spr_pizzall_bodyslamfall;
			spr_bodyslamland = spr_pizzall_bodyslamland;
			spr_bombdropstart = spr_pizzall_bodyslamstart;
			spr_bombdropfall = spr_pizzall_bodyslamfall;
			spr_bombdropland = spr_pizzall_bodyslamland;
			spr_crouch = spr_pizzall_machsliding;
			spr_bump = spr_pizzall_bump;
			spr_land = spr_pizzall_land;
			spr_land2 = spr_pizzall_land2;
			spr_lookdoor = spr_pizzall_lookdoor;
			spr_walkfront = spr_pizzall_walkfront;
			spr_victory = spr_pizzall_enterkey;
			spr_keyget = spr_pizzall_keyget;
			spr_crouchslip = spr_pizzall_machsliding;
			spr_mach1 = spr_pizzall_mach1;
			spr_mach2 = spr_pizzall_mach;
			spr_mach3player = spr_pizzall_mach4;
			spr_suplexbump = spr_pizzall_bump
			spr_rollgetup = spr_pizzall_machslideend;
			spr_mach3jump = spr_pizzall_mach3jump;
			spr_mach3hitwall = spr_pizzall_mach3hitwall;
			spr_crazyrun = spr_pizzall_crazyrun;
			spr_machslide = spr_pizzall_machsliding;
			spr_machslideend = spr_pizzall_machslideend;
			spr_machslideboost = spr_pizzall_machslideboost;
			spr_machslideboost3 = spr_pizzall_machslideboost3;
			spr_land2 = spr_pizzall_land2;
			spr_stomp = spr_pizzall_bodyslamfall;
			spr_stompprep = spr_pizzall_bodyslamstart;
			spr_crouch = spr_pizzall_machsliding;
			spr_bump = spr_pizzall_bump;
			spr_victory = spr_pizzall_enterkey;
			spr_hurt = spr_pizzall_hurt;
			spr_climbwall = spr_pizzall_climbwall;
			spr_grab = spr_pizzall_suplexdash;
			spr_mach2jump = spr_pizzall_mach2jump;
			spr_longjump = spr_pizzall_mach2jump
			spr_longjumpend = spr_pizzall_mach2jump
			spr_Timesup = spr_pizzall_timeup;
			spr_hurtjump = spr_pizzall_hurt;
			spr_blockbreak1 = spr_pizzall_blockbreak1
			spr_blockbreak2 = spr_pizzall_blockbreak2
			spr_blockbreak3 = spr_pizzall_blockbreak3
			spr_blockbreak4 = spr_pizzall_blockbreak4
			spr_blockbreak5 = spr_pizzall_blockbreak5
			spr_blockbreak6 = spr_pizzall_blockbreak6
			spr_airdash1 = spr_pizzall_airdash1;
			spr_airdash2 = spr_pizzall_airdash2;
			spr_keyget = spr_pizzall_keyget;
			spr_idle1 = spr_pizzall_idle;
			spr_idle2 = spr_pizzall_idle;
			spr_idle3 = spr_pizzall_idle;
			spr_idle4 = spr_pizzall_idle;
			spr_suplexdash = spr_pizzall_suplexdash;
			spr_taunt = spr_pizzall_taunt;
			spr_selectpal = spr_pizzall_selectpal;
			spr_cottonidle = spr_pizzall_cotton_idle;
			spr_escapeidle = spr_pizzall_escapeidle;
			spr_timesupidle = spr_pizzall_timesupidle;
			spr_facehurtup = spr_pizzall_facehurtup;
			spr_facehurt = spr_pizzall_facehurt;
			spr_shotgun_player = spr_peppino_shotgun;
			spr_swingdingend = spr_pizzall_swingdingend;
			spr_superjump = spr_pizzall_superjump;
			spr_superjump_cancelgroundedmach4 = spr_pizzano_superjump_cancelmach4grounded;
			spr_superjump_cancelgrounded = spr_pizzano_mach3;
			spr_suplexdashCancel = spr_pizzall_suplexdashCancel;
			spr_outofcontrolfall = spr_pizzall_outofcontrolfall;
			spr_metal_destroyable_escape = spr_pizzall_metalblockEscape;
			spr_player_rankS = spr_pizzall_rankS;
			spr_player_rankA = spr_pizzall_rankA;
			spr_player_rankB = spr_pizzall_rankB;
			spr_player_rankC = spr_pizzall_rankC;
			spr_player_rankD = spr_pizzall_rankD;
			spr_coneball_player = spr_pizzall_coneball;
			spr_coneball_melting_player = spr_pizzall_coneball_aggro;
			spr_cottoncreator = spr_pizzall_cottonmaker;
			spr_cotton_platform = spr_pizzall_cottonplatform_tiled;
			break;
		case "GB":
			break;
		case "RM":
			// sprites
			spr_idle = spr_rosmar_idle;
			spr_move = spr_rosmar_walk;
			spr_taunt = spr_rosmar_taunt;
			break;
	}
}
function characterpal()
{
	spr_palette = spr_pal_Pizzelle;
	palnum = 29
	ini_open((("Custom/Pizzelle_") + string(customsavedpalette)) + "_palettes.ini")
	for (var i = 0; i <= 10 ; i++)
		global.custompal_col[i] = (65536 * ini_read_real(((string(characters) + "Colors") + string(i)), "Blue", 0) + (256 * ini_read_real(((string(characters) + "Colors") + string(i)), "Green", 0) + ini_read_real(((string(characters) + "Colors") + string(i)), "Red", 0)))
	ini_close()
	set_palette(0, "Default", 0);
	set_palette(1, "Sugar", 0, 8441896, 6066975, 8628991, 14496, 5579956, 144);
	set_palette(2, "Familiar Gremlin", 0, 8446200, 1607896, 8628991, 14496, 8446200, 1607896);
	set_palette(3, "Massacre", 0, c_white, 13150344, 11056895, 2639766, c_white, c_white);
	set_palette(4, "Rivals", 0, 9394088, 7617665, 8628991, 14496, 7617665);
	set_palette(5, "Gum", 0, 14247423, 10181528, 8628991, 14496, 8198303);
	set_palette(6, "Old School", 0, c_white, 6974057, 11250603, 11250603, 6974057, 6974057, 11250603, 11250603);
	set_palette(7, "Zombified", 0, 4932824, 96, 11784798, 10567090);
	set_palette(8, "Forestation", 0, 3594902, 3625271, 10797823, 4079303, 52735, 29894);
	set_palette(9, "Lamda", 0, 13985376, 6627355, 11130197, 10525952, 248, c_white);
	set_palette(10, "Golden", 0, 1489391, 25527, 1489391, 25527, 25527, 1489391);
	set_palette(11, "Gnome Wizard", 0, 16296056, 13660176, 4771936, 28672, 9988216, 6830208, 16296056, 16308408);
	set_palette(12, "Watermelon", 0, 2967076, 2177560, 7327808, 3766554, 9340159, 9340159, 9340159, c_white);
	set_palette(13, "Purple", 0, 16273560, 16273560);
	set_palette(14, "Sunrise", 0, scribble_rgb_to_bgr($EDD77A), scribble_rgb_to_bgr($ED983A), scribble_rgb_to_bgr($FFAA83), scribble_rgb_to_bgr($A03800), scribble_rgb_to_bgr($C34726), scribble_rgb_to_bgr($642626), scribble_rgb_to_bgr($954D6D), scribble_rgb_to_bgr($BDBD87), scribble_rgb_to_bgr($262626), scribble_rgb_to_bgr($EDD77A));
	set_palette(15, "Piss", 0, 4259839, 3979494);
	set_palette(16, "Cream n' Chips", 1, spr_pattern0);
	set_palette(17, "Valiant Hero", 1, spr_pattern1);
	set_palette(18, "Dead Man's Treats", 1, spr_pattern2);
	set_palette(19, "Candy Cane", 1, spr_pattern3);
	set_palette(20, "Sweet Apple", 1, spr_pattern4);
	set_palette(21, "Space Cream", 1, spr_pattern5);
	set_palette(22, "Sourness", 1, spr_pattern6);
	set_palette(23, "Neapolitan", 1, spr_pattern7);
	set_palette(24, "Cookie Cutter", 1, spr_pattern8);
	set_palette(25, "MissingTexture", 1, spr_pattern9);
	set_palette(26, "Solid", 1, spr_pattern10);
	set_palette(27, "Choco Squares", 1, spr_pattern11);
	set_palette(28, "Cotton Candy", 1, spr_pattern12);
	set_palette(29, "Custom", 0, global.custompal_col[2], global.custompal_col[3], global.custompal_col[4], global.custompal_col[5], global.custompal_col[6], global.custompal_col[7], global.custompal_col[8], global.custompal_col[9], global.custompal_col[0], global.custompal_col[1]);
	switch (character)
	{
		case "PN":
		case "N":
			spr_palette = spr_pal_Pizzano;
			palnum = 25
			ini_open((("Custom/Pizzano_") + string(customsavedpalette)) + "_palettes.ini")
			var i = 0
			for (var i = 0; i <= 10 ; i++)
				global.custompal_col[i] = (65536 * ini_read_real(((string(characters) + "Colors") + string(i)), "Blue", 0) + (256 * ini_read_real(((string(characters) + "Colors") + string(i)), "Green", 0) + ini_read_real(((string(characters) + "Colors") + string(i)), "Red", 0)))
			ini_close()
			if (state == states.cotton || state == states.cottondrill || state == states.cottonroll)
				var outline_col = scribble_rgb_to_bgr($500050)
			else
				var outline_col = c_black
			set_palette(0, "Default", 0, c_white, 13150344, 8628991, 14496, 16293960, 16278856, 16295984, 14903400, outline_col);
			set_palette(1, "Familiar Gremlin", 1607896, 1529791, 8628991, 14496, 8446200, 5678550, 16295984, 14903400, outline_col);
			set_palette(2, "Familiar Chef", 0, 16571339, 13088161, 8628991, 14496, c_white, 9670034, 16295984, 14903400, outline_col);
			set_palette(3, "Lasagna", 0, 10607871, 6790614, 6984167, 10614, 881663, 2566034, 16295984, 14903400, outline_col);
			set_palette(4, "Spice", 0, 9670034, 7893878, 8628991, 2565782, c_red, 133, 16295984, 14903400, outline_col);
			set_palette(5, "Plumber", 0, 16295984, 13660176, 8628991, 14496, 12512, 88, 16295984, 14903400, outline_col);
			set_palette(6, "Green Apple", 0, 3700243, 2438912, 8559341, 21690, 458496, 3700243, 16295984, 14903400, outline_col);
			set_palette(7, "Grape Soda", 0, 4982461, 3212154, 15761362, 8388670, 14231934, 8388670, 16295984, 14903400, outline_col);
			set_palette(8, "Antipathic", 0, 10461182, 9733581, 14277081, 11974326, 6710886, 3355443, 16295984, 14903400, outline_col);
			set_palette(9, "Gummy Bear", 0, 5263592, 855491, 5263592, 855491, 855491, 128, 16295984, 14903400, outline_col);
			set_palette(10, "Lime", 0, 4759552, 2115584, 13695144, 4771936, 4771936, 28672, 16295984, 14903400, outline_col);
			set_palette(11, "Bear5", 0, 14994783, 5396057, 8543528, 5323032, 8879748, 6974057, 16295984, 14903400, outline_col);
			set_palette(12, "Cream n' Chips", 1, spr_pattern0, 13150344, 8628991, 14496, 16293960, 16278856, 16295984, 14903400, outline_col);
			set_palette(13, "Valiant Hero", 1, spr_pattern1, 13150344, 8628991, 14496, 16293960, 16278856, 16295984, 14903400, outline_col);
			set_palette(14, "Dead Man's Treats", 1, spr_pattern2, 13150344, 8628991, 14496, 16293960, 16278856, 16295984, 14903400, outline_col);
			set_palette(15, "Candy Cane", 1, spr_pattern3, 13150344, 8628991, 14496, 16293960, 16278856, 16295984, 14903400, outline_col);
			set_palette(16, "Sweet Apple", 1, spr_pattern4, 13150344, 8628991, 14496, 16293960, 16278856, 16295984, 14903400, outline_col);
			set_palette(17, "Space Cream", 1, spr_pattern5, 13150344, 8628991, 14496, 16293960, 16278856, 16295984, 14903400, outline_col);
			set_palette(18, "Sourness", 1, spr_pattern6, 13150344, 8628991, 14496, 16293960, 16278856, 16295984, 14903400, outline_col);
			set_palette(19, "Neapolitan", 1, spr_pattern7, 13150344, 8628991, 14496, 16293960, 16278856, 16295984, 14903400, outline_col);
			set_palette(20, "Cookie Cutter", 1, spr_pattern8, 13150344, 8628991, 14496, 16293960, 16278856, 16295984, 14903400, outline_col);
			set_palette(21, "MissingTexture", 1, spr_pattern9, 13150344, 8628991, 14496, 16293960, 16278856, 16295984, 14903400, outline_col);
			set_palette(22, "Solid", 1, spr_pattern10, 13150344, 8628991, 14496, 16293960, 16278856, 16295984, 14903400, outline_col);
			set_palette(23, "Choco Squares", 1, spr_pattern11, 13150344, 8628991, 14496, 16293960, 16278856, 16295984, 14903400, outline_col);
			set_palette(24, "Cotton Candy", 1, spr_pattern12, 13150344, 8628991, 14496, 16293960, 16278856, 16295984, 14903400, outline_col);
			set_palette(25, "Custom", 0, global.custompal_col[2], global.custompal_col[3], global.custompal_col[4], global.custompal_col[5], global.custompal_col[6], global.custompal_col[7], global.custompal_col[8], global.custompal_col[9], global.custompal_col[0], global.custompal_col[1]);
			break;
		case "G":
			spr_palette = spr_pal_Gumbob;
			palnum = 24
			ini_open((("Custom/Gumbob_") + string(customsavedpalette)) + "_palettes.ini")
			var i = 0
			for (var i = 0; i <= 10 ; i++)
				global.custompal_col[i] = (65536 * ini_read_real(((string(characters) + "Colors") + string(i)), "Blue", 0) + (256 * ini_read_real(((string(characters) + "Colors") + string(i)), "Green", 0) + ini_read_real(((string(characters) + "Colors") + string(i)), "Red", 0)))
			ini_close()
			set_palette(0, "Default", 0, c_white, 11094670, 11565048, 5644188, 248);
			set_palette(1, "Familiar Slime", c_white, 12464, 13150344, 8446200, 37088);
			set_palette(2, "Rage", 0, 11583736, 88, 12512, 14464, 248);
			set_palette(3, "Rat", 0, 8446200, 30928, 13150344, 7884848, 248);
			set_palette(4, "Retro", 0, 26632, 11073736, 49240, 26632, 248, c_white, c_white, c_white, c_white, 11073736);
			set_palette(5, "Cloudy Day", 0, 16316664, 13150344, 16316664, 13150344, 248);
			set_palette(6, "Sadness", 0, 16300248, 9457744, 16285800, 13660176, 248);
			set_palette(7, "Elite", 0, c_white, 11094670, 11461655, 9539840, 248);
			set_palette(8, "Honey", 0, 63736, 4761616, 12444, 5644188, 248, c_white, c_white, c_white, 12444);
			set_palette(9, "Doughy", 0, 14177822, 12466432, 6200319, 1861601, 248);
			set_palette(10, "Cream n' Chips", 1, spr_pattern0, 13150344);
			set_palette(11, "Valiant Hero", 1, spr_pattern1, 13150344);
			set_palette(12, "Dead Man's Treats", 1, spr_pattern2, 13150344);
			set_palette(13, "Candy Cane", 1, spr_pattern3, 13150344);
			set_palette(14, "Sweet Apple", 1, spr_pattern4, 13150344);
			set_palette(15, "Space Cream", 1, spr_pattern5, 13150344);
			set_palette(16, "Sourness", 1, spr_pattern6, 13150344);
			set_palette(17, "Neapolitan", 1, spr_pattern7, 13150344);
			set_palette(18, "Cookie Cutter", 1, spr_pattern8, 13150344);
			set_palette(19, "MissingTexture", 1, spr_pattern9, 13150344);
			set_palette(20, "Solid", 1, spr_pattern10, 13150344);
			set_palette(21, "Choco Squares", 1, spr_pattern11, 13150344);
			set_palette(22, "Cotton Candy", 1, spr_pattern12, 13150344);
			set_palette(23, "Custom", 0, global.custompal_col[2], global.custompal_col[3], global.custompal_col[4], global.custompal_col[5], global.custompal_col[6], global.custompal_col[7], global.custompal_col[8], global.custompal_col[9], global.custompal_col[0], global.custompal_col[1]);
			break;
		case "C":
			spr_palette = spr_pal_Coneboy;
			palnum = 26
			ini_open((("Custom/Coneboy_") + string(customsavedpalette)) + "_palettes.ini")
			var i = 0
			for (var i = 0; i <= 10 ; i++)
				global.custompal_col[i] = (65536 * ini_read_real(((string(characters) + "Colors") + string(i)), "Blue", 0) + (256 * ini_read_real(((string(characters) + "Colors") + string(i)), "Green", 0) + ini_read_real(((string(characters) + "Colors") + string(i)), "Red", 0)))
			ini_close()
			set_palette(0, "Default", 0, 10921727, 2763501, 4751560, 2645925, 33279, 4259839, 9958399);
			set_palette(1, "Familiar IceCream", 0, 11073784, 5802208, 11073784, 2577322, 33279, 4259839, 9958399);
			set_palette(2, "Choco", 0, 14464, 4771936, 35016, 743327, 33279, 4259839, 16316664);
			set_palette(3, "Blueberry", 0, 16296056, 15738880, 16296016, 13914657, 33279, 4259839, 16440779);
			set_palette(4, "Butter Pecan", 0, 9691371, 2147815, 8442085, 4558266, 33279, 4259839, 3271652);
			set_palette(5, "Coffee", 0, c_white, 4685773, 2647475, 2708103, 33279, 4259839, c_white);
			set_palette(6, "Mint", 0, 12973962, 7911723, 4760264, 2654373, 33279, 4259839, 14547641);
			set_palette(7, "Cherry", 0, 3421385, 2763501, 6624683, 2949204, 33279, 4259839, 13684984);
			set_palette(8, "Caramel", 0, 1287919, 30928, 1007547, 1861601, 33279, 4259839, 12638456);
			set_palette(9, "Old School", 0, 13882323, 8355711, 8947848, 6776679, 8355711, 13882323, 13355979);
			set_palette(10, "Retro", 0, 1027211, 3170864, 1027211, 3170864, 3170864, 1027211, 1027211, c_white, 3170864, 1027211);
			set_palette(11, "Silver", 0, 11575440, 12512, 11575440, 11575440, 228, 5787628, 12512);
			set_palette(12, "Cream n' Chips", 1, spr_pattern0, 13150344);
			set_palette(13, "Valiant Hero", 1, spr_pattern1, 13150344);
			set_palette(14, "Dead Man's Treats", 1, spr_pattern2, 13150344);
			set_palette(15, "Candy Cane", 1, spr_pattern3, 13150344);
			set_palette(16, "Sweet Apple", 1, spr_pattern4, 13150344);
			set_palette(17, "Space Cream", 1, spr_pattern5, 13150344);
			set_palette(18, "Sourness", 1, spr_pattern6, 13150344);
			set_palette(19, "Neapolitan", 1, spr_pattern7, 13150344);
			set_palette(20, "Cookie Cutter", 1, spr_pattern8, 13150344);
			set_palette(21, "MissingTexture", 1, spr_pattern9, 13150344);
			set_palette(22, "Solid", 1, spr_pattern10, 13150344);
			set_palette(23, "Choco Squares", 1, spr_pattern11, 13150344);
			set_palette(24, "Cotton Candy", 1, spr_pattern12, 13150344);
			set_palette(25, "Custom", 0, global.custompal_col[2], global.custompal_col[3], global.custompal_col[4], global.custompal_col[5], global.custompal_col[6], global.custompal_col[7], global.custompal_col[8], global.custompal_col[9], global.custompal_col[0], global.custompal_col[1]);
			break;
		case "S":
			spr_palette = spr_pal_Peppino
			break;
		case "PT":
		case "T":
			spr_palette = spr_pal_Noise
			break;
		case "V":
			spr_palette = spr_pal_Vigilante
			break;
		case "M":
			spr_palette = spr_pal_Pepperman
			break;
		case "RM":
			spr_palette = spr_pal_RosMar;
			break;
	}
}
function characterenv()
{
	switch character
	{
		case "N":
			break
	}
}
function charactergui()
{
	switch character
	{
		case "N":
			break
	}
}
function charactersfx()
{
	sfx_taunt = "sfx_tauntpizzelle"
	taunt_upperrange = 7
	switch character
	{
		case "PN":
		case "N":
			sfx_taunt = "sfx_tauntpizzano"
			taunt_upperrange = 8
			break;
		case "G":
			sfx_taunt = "sfx_tauntgumbob"
			taunt_upperrange = 1
			break;
		case "C":
			sfx_taunt = "sfx_tauntconeboy"
			taunt_upperrange = 2
			break;
		case "S":
			sfx_taunt = "sfx_tauntpeppino"
			taunt_upperrange = 1
			mu_escapesong = mu_peppino_escape
			mu_lap2song = mu_peppino_lap
			mu_lap5song = mu_peppino_despairy
			mu_lap10song = mu_peppino_death
			sfx_lapbell = sfx_lapbells_pep
			sfx_escapecollectbig = sfx_bellcollectbig
			sfx_escapecollect = sfx_bellcollectsmall
			sfx_collectmassive = sfx_collectgiant
			sfx_collectlarge = sfx_collectbig
			sfx_collect = sfx_collectsmall
			sfx_supertaunt = sound_supertaunt1
			break;
		case "PT":
		case "T":
			sfx_taunt = "sfx_tauntnoise"
			taunt_upperrange = 1
			break;
		case "V":
			sfx_taunt = "sfx_tauntvigilante"
			taunt_upperrange = 1
			break;
		case "M":
			sfx_taunt = "sfx_tauntpepperman"
			taunt_upperrange = 1
			break;
		case "GB":
			sfx_taunt = "sfx_tauntgustavo"
			taunt_upperrange = 1
			break;
		case "RM":
			sfx_taunt = "sfx_tauntrosmar"
			taunt_upperrange = 1
			break;
	}
	sfx_fireass = sound_fireass
	if global.newvoicelines
	{
		sfx_fireass = sfx_newfireassp
	}
}
