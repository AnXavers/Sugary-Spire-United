/// @function bnet_network_send_udp(client_id, buffer_id, size)
function bnet_network_send_udp(argument0, argument1, argument2) {

	/*  @description				A wrapper function to send a bnet encoded udp packet to a client.
...
#region Example
	/*
		...
		bnet_network_send_udp("client_id", buffer, 12);
		...
		if(_bnet_type == 1){
			...
			if(c != undefined){
				if(c == _bnet_server_local_client_map){
					...
					_bnet_network_send_udp_raw(_bnet_id, _bnet_udp_socket, c[? "ip"], c[? "udp_port"], __bnet_buffer, __bnet_new_size, c[? "ping"], _bnet_congestion_thresh);
				...
				_bnet_logger("SEND A UDP PACKET TO A CLIENT FAILED. INVALID CLIENT ID");
			}
		}
