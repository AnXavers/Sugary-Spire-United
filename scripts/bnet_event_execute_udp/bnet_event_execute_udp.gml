/// @function bnet_event_execute_udp(client_id, script_name, data)
function bnet_event_execute_udp(argument0, argument1, argument2) {
...
#region Example
	/*
		...
		bnet_event_execute_udp("client_id", "src_helloWorld", json_encode(map));
	...
	*/
	...
	with(BNET_NETWORKMANAGER.id){
		if(_bnet_type == 0){
			...
			if(__bnet_client != undefined){
				...
				if(__bnet_client == _bnet_server_local_client_map){
					...
					_bnet_network_send_udp_raw(_bnet_id, _bnet_udp_socket, __bnet_client[? "ip"], __bnet_client[? "udp_port"], _bnet_write_buffer, buffer_tell(_bnet_write_buffer), __bnet_client[? "ping"], _bnet_congestion_thresh);
		...
		_bnet_logger("UDP SCRIPT EXECUTE SENT");
	}
