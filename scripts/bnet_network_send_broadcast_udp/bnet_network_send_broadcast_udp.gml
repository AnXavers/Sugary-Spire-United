/// @function bnet_network_send_broadcast_udp(list, buffer_id, size)
function bnet_network_send_broadcast_udp(argument0, argument1, argument2) {

	/*  @description				A wrapper function to broadcast a bnet encoded udp packet to a group of clients.
...
#region Example
	/*
		...
		bnet_network_send_broadcast_udp(bnet("server"), buffer, 12);
	...
	*/
	...
	with(BNET_NETWORKMANAGER.id){
		...
		if(_bnet_type == 1){
			_bnet_network_send_udp_raw(_bnet_id, _bnet_udp_socket, _bnet_ip, _bnet_server_udp_port, __bnet_buffer, __bnet_new_size, _bnet_ping, _bnet_congestion_thresh);
		
			_bnet_logger("UDP PACKET BROADCASTED");
		}else{
			var __bnet_list	= _bnet_server_list_map[? __bnet_id];
		
			if(__bnet_list != undefined){
				if(_bnet_network_send_broadcast_udp(_bnet_id, __bnet_list, __bnet_buffer, undefined)){
				...
				_bnet_logger_client(_bnet_id, "FAILED TO BROADCAST A UDP PACKET. INVALID LIST ID");
			}
		}
